...
def test_search_endpoint():
    ...
    if data["tools"]:
        ...
        assert ecosystem in ["skills", "mcps", "npm", "pypi"], f"Unexpected ecosystem: {ecosystem}"
...
def test_tool_detail_endpoint():
    ...
    for ecosystem, name in test_cases:
        ...
        if resp.status_code == 404:
            ...
            assert returned_eco in ["skills", "mcps"], f"Expected plural ecosystem, got: {returned_eco}"
            
            print(f"  ✓ {ecosystem}/{name}: Returns valid tool data with ecosystem={returned_eco}")
