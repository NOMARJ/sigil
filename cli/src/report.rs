...
pub fn emit(
    ...
    match output_path() {
        None if format == "text" => {
            print_text(result, view);
            Ok(())
        }
        None => {
            // With no policy in play, the machine formats go through exactly
            // the printers they always did.
            let active = view.is_some_and(|v| v.active());
            match format {
                "json" if !active => crate::output::print_scan_result_json(result),
...
pub fn render(result: &ScanResult, target: &str, format: &str, view: Option<PolicyView>) -> String {
    match format {
        "json" => {
            let doc = json_document(result, view);
            format!(
                "{}\n",
...
fn print_text(result: &ScanResult, view: Option<PolicyView>) {
    use crate::output;
