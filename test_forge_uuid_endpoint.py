...
def test_uuid_endpoint():
    ...
    if data["tools"]:
        ...
        assert tool['id'] == expected_uuid, f"UUID mismatch: {tool['id']} != {expected_uuid}"
        print(f"  ✓ UUID generation is consistent")
        
        # Verify slug generation is consistent
        expected_slug = generate_tool_slug(tool['name'])
        assert tool['slug'] == expected_slug, f"Slug mismatch: {tool['slug']} != {expected_slug}"
        ...
        if uuid_resp.status_code == 200:
            ...
            assert uuid_data['name'] == tool['name'], "Tool name mismatch via UUID endpoint"
        ...
        assert 0 <= tool['trust_score'] <= 100, f"Trust score out of range: {tool['trust_score']}"
        print(f"  ✓ Trust score in valid range: {tool['trust_score']}")
        
        # Check install command format
        if tool['ecosystem'] == 'skills':
            assert tool['install_command'].startswith('npx skills add'), f"Invalid skills install command: {tool['install_command']}"
        elif tool['ecosystem'] == 'mcps':
            assert 'npm install' in tool['install_command'] or '@modelcontextprotocol' in tool['install_command'], f"Invalid MCP install command: {tool['install_command']}"
...
def test_stats_endpoint():
    ...
    print(f"  ✓ Trust distribution: high={dist['high']}, medium={dist['medium']}, low={dist['low']}, very_low={dist['very_low']}")
    
    # Check categories format
    if "categories" in data and isinstance(data["categories"], dict):
        print(f"  ✓ Categories in correct format with {len(data['categories'])} categories")
        # Check for expected category keys
        if data["categories"]:
            for key in data["categories"].keys():
                # Should be snake_case
                assert '_' in key or key.islower(), f"Category key not in snake_case: {key}"
