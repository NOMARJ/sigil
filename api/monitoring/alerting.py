...
class PagerDutyChannel(NotificationChannel):
    ...
    async def send(self, alert: Alert) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    "https://events.pagerduty.com/v2/enqueue", json=event, timeout=10.0
                )
                response.raise_for_status()
...
class AlertRule:
    ...
    async def evaluate(self) -> Optional[Alert]:
        ...
        try:
            ...
            if self.last_fired:
                cooldown_end = self.last_fired + timedelta(
            ...
            if result and (
                ...
                or (isinstance(result, (int, float)) and result > 0)
            ):
...
class AlertManager:
    ...
    async def _check_error_rate(self) -> float:
        ...
        try:
            ...
            errors = sum(
                v
                for labels, v in total_metric.items()
    ...
    async def _handle_alert(self, alert: Alert):
        ...
        if notification_tasks:
            ...
            logger.info(
                f"Alert notifications sent: {successful} successful, {failed} failed"
            )
