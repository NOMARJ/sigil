...
class ForgePerformanceMonitor:
    ...
    async def update_statistics(self):
        ...
        self.stats["p95_response_time"] = response_times[
            int(len(response_times) * 0.95)
        ]
        self.stats["p99_response_time"] = response_times[
            int(len(response_times) * 0.99)
        ]

        # Calculate rates
        time_range = current_time - self.start_time
        self.stats["requests_per_second"] = len(recent_metrics) / min(
            ...
            if stats["response_times"]:
                ...
                threshold = next(
                    (
                        ...
                        if endpoint.startswith(t.endpoint_pattern)
                    ),
                    None,
                )
                if threshold:
...
class PerformanceMonitoringMiddleware:
    ...
    async def __call__(self, request, call_next):
        ...
        try:
            ...
            response.headers["X-Performance-Status"] = self.monitor.get_health_status()[
                "status"
            ]
