...
class VersionComparator:
    ...
    async def compare_versions(
        ...
        if not await credit_service.has_credits(user_id, credit_cost):
            raise ValueError(f"Insufficient credits. Need {credit_cost} credits for version comparison.")
        ...
        try:
            ...
            summary_stats = {
                ...
                "critical_added": sum(1 for c in changes.vulnerabilities_added 
                                     if c.finding.severity == Severity.CRITICAL),
                "critical_fixed": sum(1 for c in changes.vulnerabilities_fixed 
                                    if c.finding.severity == Severity.CRITICAL),
                "high_added": sum(1 for c in changes.vulnerabilities_added 
                                if c.finding.severity == Severity.HIGH),
                "high_fixed": sum(1 for c in changes.vulnerabilities_fixed 
                                if c.finding.severity == Severity.HIGH),
            }
    ...
    def _analyze_changes(
        ...
        for key in set(base_map.keys()) & set(compare_map.keys()):
            ...
            if base_finding.severity != compare_finding.severity:
                change_type = (ChangeType.SEVERITY_INCREASED 
                ...
                change = SecurityChange(
                    ...
                    author=None,
                    timestamp=None,
                    description=(f"Severity changed from {base_finding.severity.value} "
    ...
    def _build_timeline_data(self, changes: Any) -> List[Dict[str, Any]]:
        ...
        for change in changes.severity_changes:
            timeline.append({
                ...
                "old_severity": change.old_finding.severity.value if change.old_finding else None,
