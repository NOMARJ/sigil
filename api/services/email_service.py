...
class EmailService:
    ...
    async def subscribe_email(self, request: EmailSubscriptionRequest) -> EmailSubscriptionResponse:
            ...
            if existing:
                if existing['is_active']:
                    ...
                    await db.execute("""
                        ...
                        WHERE email = $2
                    """, json.dumps(request.preferences), request.email)
            else:
    ...
    async def unsubscribe_email(self, request: UnsubscribeRequest) -> UnsubscribeResponse:
        ...
        async with get_database_client() as db:
            ...
            await db.execute("""
                ...
                VALUES ($1, $2, $3)
            """, subscription['email'], request.token, request.reason)
    ...
    async def generate_weekly_digest(self, week_ending: datetime) -> WeeklyDigestContent:
        ...
        async with get_database_client() as db:
            ...
            community_highlights = [
                {"title": "Community Spotlight", "content": "Featured community contributions and discussions"},
                {"title": "Tool of the Week", "content": "Community-voted tool recommendation"}
            ]
    ...
    async def create_email_campaign(self, request: EmailCampaignRequest) -> EmailCampaignResponse:
    ...
    async def _send_campaign(self, campaign_id: str) -> None:
        ...
        async with get_database_client() as db:
            ...
            # Generate HTML email
            html_content = await self._render_email_template('weekly_digest.html', {
                ...
                for j, result in enumerate(results):
                    ...
                    if not isinstance(result, Exception):
                        await db.execute("""
                            ...
                            VALUES ($1, $2, $3)
                        """, campaign_id, subscriber['email'], 'sent')
                        sent_count += 1
                    else:
                        ...
                        await db.execute("""
                            INSERT INTO email_sends (campaign_id, email, status)
                            VALUES ($1, $2, $3)
                        """, campaign_id, subscriber['email'], 'failed')
    ...
    async def _send_welcome_email(self, email: str, unsubscribe_token: str) -> None:
        ...
        html_content = await self._render_email_template('welcome.html', {
            'email': email,
            'unsubscribe_url': unsubscribe_url,
            'base_url': self.base_url
        })
