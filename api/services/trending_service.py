...
class TrendingService:
    ...
    def _get_date_range(self, timeframe: TimeFrame, reference_date: Optional[date] = None) -> Tuple[date, date]:
    ...
    def _get_previous_date_range(self, timeframe: TimeFrame, reference_date: Optional[date] = None) -> Tuple[date, date]:
    ...
    async def _get_previous_rankings(
        ...
        try:
            async with get_database_client() as db:
                ...
                return {row["tool_id"]: row["rank_position"] for row in rows} if rows else {}
    ...
    async def calculate_trending_scores(
        ...
        try:
            # Fetch raw metrics
            metrics_data = await self._fetch_tool_metrics(timeframe, ecosystem, category, limit * 2)
            
            if not metrics_data:
                self.logger.warning(f"No metrics data found for {timeframe}/{ecosystem}/{category}")
                return []
            
            # Calculate max values for normalization
            max_downloads = max((row["current_downloads"] for row in metrics_data), default=1)
            max_stars = max((row["current_stars"] for row in metrics_data), default=1)
            
            # Get previous rankings for rank change calculation
            previous_rankings = await self._get_previous_rankings(timeframe, ecosystem, category)
            ...
            for row in metrics_data:
                ...
                downloads_growth = self._calculate_growth_rate(current_downloads, previous_downloads)
                stars_growth = self._calculate_growth_rate(current_stars, previous_stars)
            ...
            if duration > 50:
                self.logger.warning(f"Trending calculation exceeded 50ms target: {duration:.2f}ms")
    ...
    async def get_trending_tools(
        ...
        if use_cache:
            ...
            cached_results = await self._get_from_cache(timeframe, ecosystem, category, limit)
            if cached_results:
                return cached_results
        
        # Calculate fresh trending data
        trending_data = await self.calculate_trending_scores(timeframe, ecosystem, category, limit)
