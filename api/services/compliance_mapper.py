...
class ComplianceMapper:
    ...
    def _load_frameworks(self) -> Dict[str, Any]:
        ...
        try:
            data_path = (
                Path(__file__).parent.parent / "data" / "compliance_frameworks.json"
            )
    ...
    async def map_findings_to_compliance(
        ...
        credits_needed = 3
        has_credits = await self.credit_service.check_balance(user_id, credits_needed)

        if not has_credits:
            return {"error": "Insufficient credits", "credits_needed": credits_needed}
                    ...
                    for framework_ref in mapping.get("frameworks", []):
                        ...
                        if framework_name in report["violations"]:
                            violation = {
                                ...
                                "remediation_priority": mapping.get(
                            ...
                            if framework_name in self.frameworks_data["frameworks"]:
                                fw_data = self.frameworks_data["frameworks"][
                                    framework_name
                                ]
                                ...
                                if (
                                    ...
                                    and category in fw_data["categories"]
                                ):
                                    violation["category_name"] = fw_data["categories"][
                                        category
                                    ]["name"]
                                    violation["category_description"] = fw_data[
                                        "categories"
                                    ...
                                    and category in fw_data.get("requirements", {})
                                ):
                                    violation["category_name"] = fw_data[
                                        "requirements"
                                    ][category]["name"]
                                elif (
                                    framework_name == "HIPAA"
                                    and category in fw_data.get("safeguards", {})
                                ):
                                    violation["category_name"] = fw_data["safeguards"][
                                        category
                                    ]["name"]
                                elif (
                                    framework_name == "GDPR"
                                    and category in fw_data.get("articles", {})
                                ):
                                    violation["category_name"] = fw_data["articles"][
                                        category
                                    ]["name"]
                                    violation["category_description"] = fw_data[
                                        "articles"
                ...
                if violations:
                    report["summary"][framework] = {
                        ...
                        "categories_affected": len(
                            set(v["category"] for v in violations)
                        ),
                    ...
                    report["remediation_priorities"].append(
                        {
                            ...
                            "frameworks_violated": self._get_frameworks_for_pattern(
                                violation["pattern"]
                            ),
    ...
    def _adjust_severity(
        self, base_severity: str, framework: str, compliance_context: Optional[str]
    ) -> str:
    ...
    def _get_frameworks_for_pattern(self, pattern: str) -> List[str]:
        ...
        if pattern in self.frameworks_data["pattern_mappings"]:
            frameworks = self.frameworks_data["pattern_mappings"][pattern].get(
                "frameworks", []
            )
    ...
    async def generate_compliance_report_markdown(self, report: Dict[str, Any]) -> str:
        ...
        lines.extend(["## Detailed Violations by Framework", ""])
            ...
            if violations:
                ...
                for category, cat_violations in categories.items():
                    ...
                    if len(cat_violations) > 5:
                        lines.append(
                            f"- _{len(cat_violations) - 5} more violations..._"
                        )
                    lines.append("")
