...
class ForgeAnalyticsService:
    ...
    async def track_event(
        ...
        try:
            ...
            # Store in database
            await db.insert("forge_analytics_events", {
    ...
    async def get_organization_analytics(
        ...
        try:
            ...
            analytics_data = OrganizationAnalyticsResponse(
                ...
                cost_optimization_opportunities=await self._identify_cost_optimizations(org_teams),
                organization_risk_score=sum(risk_scores) / len(risk_scores) if risk_scores else 100.0,
    ...
    async def _update_realtime_caches(self, event: ForgeAnalyticsEvent):
        ...
        try:
            ...
            if event.event_type in [ForgeEventType.TOOL_VIEWED, ForgeEventType.TOOL_TRACKED]:
    ...
    async def _update_user_tools_table(self, event: ForgeAnalyticsEvent):
        ...
        if existing:
            ...
            # Insert new record
            await db.insert("forge_user_tools", {
                "user_id": event.user_id,
                "tool_id": tool_data["tool_id"],
                "ecosystem": tool_data["ecosystem"],
    ...
    async def _update_user_stacks_table(self, event: ForgeAnalyticsEvent):
        ...
        await db.insert("forge_user_stacks", {
            "user_id": event.user_id,
    ...
    async def _aggregate_personal_analytics(
        ...
        most_viewed_list = [
            {"tool_key": k, "count": v} 
            for k, v in sorted(most_viewed_tools.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
        most_tracked_list = [
            {"tool_key": k, "count": v}
            for k, v in sorted(most_tracked_tools.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
    ...
    async def _calculate_team_security_score(self, team_id: str) -> float:
        ...
        try:
            ...
            result = await db.execute_raw(
                "SELECT dbo.fn_get_security_compliance_score(?) as score",
                (team_id,)
            )
            if result:
    ...
    async def _calculate_cost_estimates(self, teams: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    async def _identify_cost_optimizations(self, teams: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _get_high_risk_tools(self, teams: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Get high-risk tools across organization."""
        return []  # TODO: Implement based on public_scans
    
    async def _get_compliance_metrics(self, teams: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    async def _get_security_trends(self, teams: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
