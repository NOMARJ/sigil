...
class AnalyticsService:
    ...
    async def track_llm_usage(
        ...
        try:
            ...
            cost_cents = cost_override or self._calculate_llm_cost(model_used, tokens_used)
            
            # Analyze insights to extract metrics
            threats_found = sum(1 for insight in insights_generated if insight.get('confidence', 0) > 0.7)
            confidence_avg = (
                sum(insight.get('confidence', 0) for insight in insights_generated) / len(insights_generated)
                if insights_generated else 0.0
            )
            ...
            """, (
                user_id, scan_id, model_used, tokens_used, processing_time_ms,
                float(cost_cents), len(insights_generated), threats_found, confidence_avg,
                cache_hit, fallback_used
            ))
            ...
            await self._update_daily_engagement(user_id, {
                "scans_performed": 1,
                "threats_discovered": threats_found,
                "llm_tokens_used": tokens_used
            })
            
            logger.debug(f"Tracked LLM usage for user {user_id}: {tokens_used} tokens, {threats_found} threats")
    ...
    async def track_threat_discovery(
        ...
        try:
            ...
            """, (
                user_id, scan_id, threat_type, severity, confidence, is_zero_day,
                file_path, threat_hash, analysis_type, evidence_snippet,
            ...
            logger.debug(f"Tracked threat discovery: {threat_type} ({severity}) for user {user_id}")
            
        except Exception as e:
            logger.exception(f"Failed to track threat discovery for user {user_id}: {e}")
    ...
    async def track_event(
        ...
        try:
            ...
            logger.exception(f"Failed to track event {event_type} for user {user_id}: {e}")
    ...
    async def get_daily_usage_report(
        ...
        try:
            ...
            llm_stats = await db.fetch_one(f"""
                SELECT 
            ...
            zero_day_stats = await db.fetch_one(f"""
            ...
            cache_hit_rate = (llm_stats.get('cache_hits', 0) or 0) / total_scans * 100 if total_scans > 0 else 0
            fallback_rate = (llm_stats.get('fallbacks_used', 0) or 0) / total_scans * 100 if total_scans > 0 else 0
            ...
            return DailyUsageReport(
                ...
                total_cost=Decimal(str(llm_stats.get('total_cost_cents', 0) or 0)) / 100,
    ...
    async def get_user_churn_risk(self, user_id: str) -> ChurnRiskMetrics:
        ...
        try:
            ...
            avg_session_duration = engagement_stats.get('avg_session_duration', 0.0) or 0.0
            ...
            feature_adoption_score = min(unique_features / 5.0, 1.0)  # Assume 5 main features
    ...
    async def get_user_usage_stats(
        ...
        try:
            ...
            return UserUsageStats(
                ...
                cost_this_period=Decimal(str(period_stats.get('cost_cents', 0) or 0)) / 100,
    ...
    async def _update_daily_engagement(
        ...
        try:
            ...
            logger.exception(f"Failed to update daily engagement for user {user_id}: {e}")
