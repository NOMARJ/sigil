...
class RealTimeDashboardService:
    ...
    async def connect_websocket(
        ...
        logger.info(f"WebSocket connected: {connection_id} with subscriptions {subscriptions}")
    ...
    async def _send_initial_data(
        ...
        try:
            ...
            if "dashboard_stats" in subscriptions:
                initial_data["data"]["dashboard_stats"] = await self._get_dashboard_stats(user_id)
            
            if "realtime_events" in subscriptions:
                initial_data["data"]["recent_events"] = await self._get_recent_events(user_id)
    ...
    async def _send_to_websockets(self, update_message: Dict[str, Any]):
        ...
        for connection_id, websocket in self.active_connections.items():
            ...
            if update_type in ["personal_analytics", "team_analytics"] and "analytics" not in user_subs:
                should_send = False
    ...
    async def _get_dashboard_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "avg_risk_score": sum(s.get("risk_score", 0) for s in recent_scans) / max(len(recent_scans), 1),
                "high_risk_count": len([s for s in recent_scans if s.get("risk_score", 0) > 50]),
                "last_scan": recent_scans[0].get("created_at") if recent_scans else None
            }
...
async def send_security_notification(user_id: str, message: str, severity: str = "medium"):
