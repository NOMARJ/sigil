...
class BulkAnalyzer:
    ...
    async def _analyze_pattern_group(
        self, user_id: str, pattern_type: str, group: Dict[str, Any], depth: str
    ) -> Dict[str, Any]:
    ...
    def _estimate_credits(self, pattern_groups: Dict[str, Dict], depth: str) -> int:
        """Estimate total credits needed for analysis"""
        base_costs = {"quick": 3, "thorough": 5, "exhaustive": 10}
    ...
    def _generate_summary(self, analyzed_groups: Dict[str, Dict]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(
        self, pattern_groups: Dict[str, Dict], analyzed_groups: Dict[str, Dict]
    ) -> List[Dict[str, Any]]:
        ...
        for pattern_type, group in analyzed_groups.items():
            if group.get("single_fix_possible") and "CRITICAL" in group.get(
                ...
                recommendations.append(
                    {
                        ...
                        "description": group.get("fix_suggestion", {}).get(
                            "description", ""
                        ),
                        "impact": "Critical security vulnerabilities",
                        "effort": group.get("fix_suggestion", {}).get(
        ...
        for pattern_type, group in analyzed_groups.items():
            if (
                group.get("likely_same_root_cause")
                and "HIGH" in group.get("severity_distribution", {})
                and pattern_type not in [r["pattern"] for r in recommendations]
            ):
                recommendations.append(
                    {
                        "priority": 2,
                        ...
                        "effort": self._estimate_effort_from_count(
        ...
        for pattern_type, group in analyzed_groups.items():
            ...
            if (
                fix
                and "Low" in fix.get("estimated_effort", "")
                and group["findings_count"] >= 3
                and pattern_type not in [r["pattern"] for r in recommendations]
            ):
                recommendations.append(
                    {
