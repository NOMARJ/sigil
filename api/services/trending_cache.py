...
class TrendingCacheService:
    ...
    def _deserialize_trending_data(self, cached_data: str) -> Optional[List[TrendingMetrics]]:
        ...
        try:
            ...
            self.logger.debug(f"Deserialized {len(trending_list)} trending tools from cache")
    ...
    async def _store_to_database_cache(
        ...
        try:
            ...
            for metrics in trending_data:
                ...
                """,
                metrics.tool_id, timeframe, ecosystem, category, metrics.rank_position,
                metrics.previous_rank, metrics.rank_change, metrics.downloads_growth,
                metrics.direction, metrics.composite_score,
                metrics.current_downloads, metrics.previous_downloads, metrics.downloads_growth,
                metrics.current_stars, metrics.previous_stars, metrics.stars_growth,
                metrics.current_trust_score, cache_key, expires_at,
                datetime.utcnow(), datetime.utcnow())
            
            self.logger.debug(f"Stored {len(trending_data)} trending entries in database cache")
            ...
            if hasattr(cache, '_client') and cache._client is not None:
                try:
                    ...
                    if keys:
                        ...
                        self.logger.info(f"Invalidated {len(keys)} cache keys matching pattern: {pattern}")
                except Exception as redis_error:
                    self.logger.warning(f"Redis cache invalidation failed: {redis_error}")
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "redis_connected": cache._connected if hasattr(cache, '_connected') else False,
