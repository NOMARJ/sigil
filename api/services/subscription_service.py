...
class SubscriptionService:
    ...
    async def get_user_subscription(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            result = await db.execute_procedure(
                "sp_GetUserSubscription", {"user_id": user_id}
            )
            if result:
            ...
            return await db.get_subscription(
                user_id
            ) or self._default_free_subscription(user_id)
    ...
    async def get_user_tier(self, user_id: str) -> PlanTier:
        ...
        try:
            ...
            logger.warning(
                f"Invalid plan tier '{plan_str}' for user {user_id}, defaulting to FREE"
            )
    ...
    async def create_pro_subscription(
        ...
        try:
            ...
            logger.exception(
                f"Failed to create Pro subscription for user {user_id}: {e}"
            )
    ...
    async def cancel_pro_subscription(
        self, user_id: str, cancellation_reason: str = "user_cancelled"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if result:
                logger.info(
                    f"Successfully cancelled Pro subscription for user {user_id}"
                )
                return result[0]
            else:
            ...
            logger.exception(
                f"Failed to cancel Pro subscription for user {user_id}: {e}"
            )
    ...
    async def track_pro_feature_usage(
        self, user_id: str, feature_type: str, usage_data: dict[str, Any] | None = None
    ) -> bool:
        ...
        try:
            ...
            if result and result[0].get("records_inserted", 0) > 0:
                ...
                logger.warning(
                    f"Failed to track {feature_type} usage for user {user_id}"
                )
                return False
