...
class ContextExpander:
    ...
    async def expand_context(
        ...
        if not await credit_service.has_credits(user_id, credit_cost):
            raise ValueError(
                f"Insufficient credits. Need {credit_cost} credits for context expansion."
            )
        ...
        try:
            ...
            if expansion_type == "custom" and specific_files:
                ...
                files_to_add = await self._determine_files_to_expand(
                    scan_path, original_files, expansion_type
                )

            # Load the additional files
            expanded_context = await self._load_expanded_files(scan_path, files_to_add)

            # Re-analyze with expanded context
            findings_after = await self._reanalyze_with_context(
                initial_findings, expanded_context, user_id
            )

            # Compare assessments
            assessment_changes = self._compare_assessments(
                initial_findings, findings_after
            )
            ...
            context_size = (
                sum(len(content) for content in expanded_context.values()) / 1024
            )
    ...
    async def _determine_files_to_expand(
        self, scan_path: str, original_files: List[str], expansion_type: str
    ) -> List[str]:
    ...
    async def _find_imported_files(
        self, scan_path: str, original_files: List[str]
    ) -> List[str]:
    ...
    async def _load_expanded_files(
        self, scan_path: str, files: List[str]
    ) -> Dict[str, str]:
            ...
            if full_path.exists() and full_path.is_file():
                try:
                    ...
                    if full_path.stat().st_size < 500_000:  # 500KB limit
                        content = full_path.read_text(errors="ignore")
                        expanded_context[file_path] = content[
                            :50000
                        ]  # Limit to 50K chars
                except Exception as e:
    ...
    def _update_finding_with_context(self, finding: Finding, response: Any) -> Finding:
        ...
        for insight in response.insights:
            if (
                ...
                and finding.file in insight.affected_files
                and self._rules_match(finding.rule, insight.title)
            ):
                ...
                if insight.false_positive_likelihood > 0.7:
                    ...
                    finding.description = (
                        f"[LIKELY FALSE POSITIVE] {finding.description}"
                    )
                    ...
                    finding.description = (
                        f"[CONFIRMED WITH CONTEXT] {finding.description}"
                    )

                # Add context information
                if insight.reasoning:
                    finding.explanation = (
                        f"{finding.explanation or ''} | Context: {insight.reasoning}"
                    )
    ...
    def _compare_assessments(
        self, before: List[Finding], after: List[Finding]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_confidence_improvement(
        ...
        false_positives_before = sum(
            1 for f in before if "false" in f.description.lower()
        )
        false_positives_after = sum(
            1 for f in after if "false" in f.description.lower()
        )
    ...
    async def _store_expansion(
        self, session_id: str, expansion: ContextExpansion
    ) -> None:
        ...
        try:
            ...
            await db.execute(
                ...
                """,
                {"session_id": session_id, "expansion": json.dumps(expansion_data)},
            )
        except Exception as e:
