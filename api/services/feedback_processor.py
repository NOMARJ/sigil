...
from ..models.suppression_rules import (
    FeedbackType,
    SuppressionScope,
    SuppressionRule,
    UserFeedback,
...
class FeedbackProcessor:
    ...
    async def process_feedback(
        ...
        try:
            ...
            if feedback_type in [FeedbackType.FALSE_POSITIVE, FeedbackType.TRUE_POSITIVE]:
                await self._process_suppression_rules(feedback, finding)
            
            # Update aggregated metrics
            await self._update_aggregations(feedback)
            
            logger.info(f"Processed feedback {feedback.feedback_id} for finding {finding.id}")
            return feedback
    ...
    async def _update_rule(
        ...
        if feedback.feedback_type == existing_rule.original_feedback:
            ...
            existing_rule.effectiveness_score = min(
                1.0,
                existing_rule.effectiveness_score + 0.1
            )
            ...
            if feedback.feedback_type == FeedbackType.FALSE_POSITIVE:
                existing_rule.confidence_adjustment = max(
                    -1.0,
                    existing_rule.confidence_adjustment - 0.1
                )
        else:
            # Contradictory feedback weakens the rule
            existing_rule.effectiveness_score = max(
                0.0,
                existing_rule.effectiveness_score - 0.2
            )
    ...
    async def apply_suppression_rules(
        ...
        try:
                ...
                for rule in rules:
                    if self._rule_matches_finding(rule, finding):
                        ...
                        if rule.suppress_completely and rule.active:
                            suppressed = True
                            logger.debug(f"Suppressed finding {finding.id} by rule {rule.rule_id}")
                            break
                        elif rule.active:
                ...
                if not suppressed:
                    ...
                    if confidence_adjustment != 0:
                        finding.confidence = max(
                            0.0,
                            min(1.0, finding.confidence + confidence_adjustment)
                        )
            ...
            logger.info(f"Applied suppression rules: {len(findings)} -> {len(processed_findings)} findings")
    ...
    async def get_accuracy_metrics(
        ...
        try:
            ...
            pattern_accuracy = await self._get_pattern_accuracy(user_id, team_id, start_date)
            
            # Get suppression effectiveness
            suppression_stats = await self._get_suppression_stats(user_id, team_id, start_date)
            ...
            metrics = AccuracyMetrics(
                ...
                feedback_rate=total_feedback / stats['total_findings'] if stats['total_findings'] > 0 else 0,
