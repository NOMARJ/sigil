...
class ForgeUserToolsService:
    ...
    async def get_user_tool_recommendations(
        ...
        try:
            ...
            tracked_tool_ids = {
                f"{t['ecosystem']}:{t['tool_id']}" for t in tracked_tools
            }
            ...
            for candidate in candidates:
                ...
                if tool_key not in tracked_tool_ids:
                    ...
                    rec_score = self._calculate_recommendation_score(
                        candidate, user_ecosystems, user_categories, trust_score
                    )
    ...
    async def get_usage_statistics(
        ...
        try:
            ...
            events = await db.select("forge_analytics_events", {"user_id": user_id})

            # Filter events by date (simplified)
            recent_events = [
                e for e in events if e.get("timestamp", cutoff_date) >= cutoff_date
            ]
            ...
            stats = {
                ...
                "tools_starred": len(
                    [t for t in tracked_tools if t.get("is_starred", False)]
                ),
            ...
            for event in recent_events:
                ...
                stats["event_breakdown"][event_type] = (
                    stats["event_breakdown"].get(event_type, 0) + 1
                )

            # Ecosystem distribution
            for tool in tracked_tools:
                ecosystem = tool["ecosystem"]
                stats["ecosystem_distribution"][ecosystem] = (
                    stats["ecosystem_distribution"].get(ecosystem, 0) + 1
                )
    ...
    def _get_recommendation_reason(
        self, candidate: Dict[str, Any], user_ecosystems: set, user_categories: set
    ) -> str:
