...
class TestTrendingService:
    ...
    def test_determine_trend_direction(self, trending_service):
        ...
        # New tool
        assert trending_service._determine_trend_direction(0, 0, is_new_tool=True) == "new"
        
        # Strong growth
        assert trending_service._determine_trend_direction(50.0, 5, is_new_tool=False) == "up"
        
        # Strong decline  
        assert trending_service._determine_trend_direction(-50.0, -5, is_new_tool=False) == "down"
        
        # Stable
        assert trending_service._determine_trend_direction(10.0, 0, is_new_tool=False) == "stable"
        
        # Rank improvement trumps low growth
        assert trending_service._determine_trend_direction(5.0, 3, is_new_tool=False) == "up"
    ...
    async def test_fetch_tool_metrics(self, trending_service, mock_db_data):
        ...
        with patch('trending_service.get_database_client') as mock_db:
            mock_db.return_value.__aenter__.return_value.fetch = AsyncMock(return_value=mock_db_data)
            
            metrics = await trending_service._fetch_tool_metrics("7d", "all", "all", 100)
    ...
    async def test_calculate_trending_scores_performance(self, trending_service):
        ...
        with patch.object(trending_service, '_fetch_tool_metrics') as mock_fetch:
            ...
            with patch.object(trending_service, '_get_previous_rankings') as mock_rankings:
                mock_rankings.return_value = {}
                
                import time
                start_time = time.time()
                
                results = await trending_service.calculate_trending_scores("7d", "all", "all", 100)
                
                duration = (time.time() - start_time) * 1000  # Convert to ms
                
                # Should complete in under 50ms for 1000 tools
                assert duration < 50, f"Performance requirement failed: {duration:.2f}ms > 50ms"
                assert len(results) == 100  # Limited to requested amount
                assert results[0].rank_position == 1
                assert results[-1].rank_position == 100
    
    @pytest.mark.asyncio
    async def test_calculate_trending_scores_integration(self, trending_service, mock_db_data):
        """Integration test for trending scores calculation."""
        with patch.object(trending_service, '_fetch_tool_metrics') as mock_fetch:
            mock_fetch.return_value = mock_db_data
            
            with patch.object(trending_service, '_get_previous_rankings') as mock_rankings:
                mock_rankings.return_value = {"tool1": 3, "tool2": 1}  # Previous rankings
                
                results = await trending_service.calculate_trending_scores("7d", "npm", "all", 10)
    ...
    async def test_get_trending_tools_no_cache(self, trending_service, mock_db_data):
        ...
        with patch.object(trending_service, 'calculate_trending_scores') as mock_calc:
            ...
            results = await trending_service.get_trending_tools("7d", "all", "all", 20, use_cache=False)
    ...
    with patch.object(service, '_fetch_tool_metrics') as mock_fetch:
        ...
        with patch.object(service, '_get_previous_rankings') as mock_rankings:
            ...
            for limit in [100, 500, 1000]:
                start_time = time.time()
                results = await service.calculate_trending_scores("7d", "all", "all", limit)
