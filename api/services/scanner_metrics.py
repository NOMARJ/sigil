...
class ScannerMetricsCollector:
    ...
    async def _get_version_metrics(
        self, version_prefix: str, cutoff_date: datetime
    ) -> ScannerMetrics | None:
        ...
        try:
            ...
            rows = await db.execute_raw_sql(
                version_query, (f"{version_prefix}.%", cutoff_date)
            )
            ...
            for row in rows:
                ...
                verdict_dist[verdict] = (
                    verdict_dist.get(verdict, 0) + row["verdict_count"]
                )
            ...
            logger.exception(
                "Failed to get metrics for version %s: %s", version_prefix, e
            )
            return None

    def _estimate_false_positive_rate(
        self, verdict_dist: Dict[str, int], version: str
    ) -> float | None:
    ...
    async def _get_migration_statistics(self, cutoff_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            v1_scans = sum(
                row["count"] for row in rows if row["version"].startswith("1.")
            )
            v2_scans = sum(
                row["count"] for row in rows if row["version"].startswith("2.")
            )

            migration_percentage = (
                (v2_scans / total_scans * 100) if total_scans > 0 else 0.0
            )
            ...
            if rows:
                ...
                if avg_original > 0:
                    improvement_percentage = (
                        (avg_original - avg_new) / avg_original
                    ) * 100
    ...
    def _calculate_improvements(
        self, v1_metrics: ScannerMetrics | None, v2_metrics: ScannerMetrics | None
    ) -> Dict[str, Any]:
        ...
        if v1_metrics.average_score > 0:
            score_improvement = (
                (v1_metrics.average_score - v2_metrics.average_score)
                / v1_metrics.average_score
            ) * 100

        fp_improvement = 0.0
        if v1_metrics.false_positive_rate and v2_metrics.false_positive_rate:
            fp_improvement = (
                v1_metrics.false_positive_rate - v2_metrics.false_positive_rate
            )
        ...
        return {
            ...
            "confidence_improvement": {
                ...
                "improvement": v2_metrics.average_confidence
                - (v1_metrics.average_confidence or 0)
                if v2_metrics.average_confidence
                else None,
            },
        }
    ...
    def _generate_summary(
        self,
        v1_metrics: ScannerMetrics | None,
        v2_metrics: ScannerMetrics | None,
        ...
        if not improvements.get("available"):
            return {
                ...
                "message": "Not enough data to generate meaningful comparison",
            }
