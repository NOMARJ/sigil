...
class PatternGrouper:
    ...
    def group_findings(self, findings: List[Finding]) -> Dict[str, Dict[str, any]]:
        """
        ...
        for pattern_type, group in groups.items():
            if group["count"] > 0:
                ...
                group["likely_same_root_cause"] = (
                    group["root_cause_similarity"] > 0.7
                    and len(group["common_characteristics"].get("common_patterns", []))
                    > 0
                )
    ...
    def _classify_pattern(self, finding: Finding) -> Optional[str]:
        ...
        for pattern_type, config in self.PATTERN_TYPES.items():
            ...
            for indicator in config["indicators"]:
                if (
                    indicator.lower() in evidence_lower
                    or indicator.lower() in description_lower
                ):
                    return pattern_type
    ...
    def _analyze_common_characteristics(
        ...
        if all_evidence:
            ...
            characteristics["common_variables"] = [
                var for var, count in var_counts.items() if count >= len(findings) * 0.5
            ]
        ...
        common_words = self._find_common_words([f.description for f in findings])
        characteristics["common_patterns"] = common_words

        return characteristics

    def _calculate_root_cause_similarity(self, findings: List[Finding]) -> float:
        ...
        for i in range(len(findings)):
            for j in range(i + 1, len(findings)):
                ...
                if f1.evidence and f2.evidence:
                    evidence_sim = self._string_similarity(f1.evidence, f2.evidence)
    ...
    def _find_common_words(self, texts: List[str], min_length: int = 4) -> List[str]:
    ...
    def suggest_single_fix(
        self, pattern_group: Dict[str, any]
        ...
        common_funcs = pattern_group["common_characteristics"].get(
            "common_functions", []
        )
        common_vars = pattern_group["common_characteristics"].get(
            "common_variables", []
        )
