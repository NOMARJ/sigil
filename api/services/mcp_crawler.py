...
POPULAR_MCP_SERVERS = {
    "mcp-postgres", "mcp-mongodb", "mcp-redis", "mcp-mysql", "mcp-sqlite", 
    "mcp-github", "mcp-gitlab", "mcp-jira", "mcp-slack", "mcp-discord",
    "mcp-stripe", "mcp-shopify", "mcp-aws", "mcp-gcp", "mcp-azure",
    "mcp-docker", "mcp-kubernetes", "mcp-terraform", "mcp-ansible",
    "mcp-jupyter", "mcp-vscode", "mcp-obsidian", "mcp-notion"
}
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        try:
            ...
            if remaining < 10:
                ...
                logger.warning("GitHub rate limit low (%d remaining), waiting %ds", 
                             remaining, wait_time)
...
async def detect_typosquats(servers: list[MCPServer]) -> list[TyposquatAlert]:
    ...
    alerts.sort(key=lambda a: (risk_order[a.risk_level], a.similarity_score), reverse=True)
...
async def store_mcp_results(results: list[MCPScanResult]) -> int:
    ...
    for result in results:
        ...
        try:
            ...
            logger.exception("Failed to store MCP scan result: %s", result.server.repo_name)
...
async def _main() -> None:
    ...
    parser.add_argument("--typosquat-check", action="store_true", help="Check for typosquats")
    parser.add_argument("--server", help="Scan specific MCP server (user/repo)")
    parser.add_argument("--max-pages", type=int, default=10, help="Max GitHub search pages")
