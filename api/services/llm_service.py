...
class LLMService:
    ...
    async def analyze_threat(self, request: LLMAnalysisRequest) -> LLMAnalysisResponse:
        ...
        if not llm_config.is_configured():
            ...
            return self._create_fallback_response(analysis_id, "LLM service not configured")
    ...
    async def _perform_analysis(self, request: LLMAnalysisRequest, analysis_id: str) -> LLMAnalysisResponse:
        ...
        context_analysis = self._parse_context_analysis(llm_response, request) if request.include_context_analysis else None
    ...
    def _parse_llm_insights(self, llm_response: str, request: LLMAnalysisRequest) -> list[LLMInsight]:
            ...
            for insight_data in data.get("insights", [])[:request.max_insights]:
                try:
                    insight = LLMInsight(
                        ...
                        threat_category=LLMThreatCategory(insight_data["threat_category"]),
                        confidence=insight_data["confidence"],
                        confidence_level=confidence_to_level(insight_data["confidence"]),
                        ...
                        severity_adjustment=insight_data.get("severity_adjustment", 0.0),
                        false_positive_likelihood=insight_data.get("false_positive_likelihood", 0.0),
                        remediation_suggestions=insight_data.get("remediation_suggestions", []),
                        mitigation_steps=insight_data.get("mitigation_steps", [])
                    )
                    insights.append(insight)
                except (KeyError, ValueError, TypeError) as e:
                    logger.warning(f"Failed to parse insight: {e}, data: {insight_data}")
                    continue
    ...
    def _parse_context_analysis(self, llm_response: str, request: LLMAnalysisRequest) -> LLMContextAnalysis | None:
    ...
    async def _get_cached_analysis(self, analysis_id: str) -> LLMAnalysisResponse | None:
    ...
    async def _cache_analysis(self, analysis_id: str, response: LLMAnalysisResponse) -> None:
        """Cache analysis result."""
        try:
            ttl_seconds = llm_config.cache_ttl_hours * 3600
            await db.set_cache(
                f"llm_analysis:{analysis_id}", 
                response.model_dump(), 
                ttl=ttl_seconds
            )
        except Exception as e:
    ...
    def _create_fallback_response(self, analysis_id: str, error_message: str) -> LLMAnalysisResponse:
