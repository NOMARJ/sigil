...
class ForgeMatcher:
    ...
    async def find_env_var_matches(self, classification_id: str) -> list[ToolMatch]:
        ...
        for candidate in candidates:
            ...
            candidate_env_vars = set(
                json.loads(candidate.get("environment_vars", "[]"))
            )
            shared_env_vars = tool_env_vars.intersection(candidate_env_vars)

            if shared_env_vars:
                # Get trust scores from public_scans
                primary_trust = await self._get_trust_score(
                    tool["ecosystem"], tool["package_name"]
                )
                secondary_trust = await self._get_trust_score(
                    candidate["ecosystem"], candidate["package_name"]
                )
    ...
    async def find_protocol_matches(self, classification_id: str) -> list[ToolMatch]:
        ...
        for candidate in candidates:
            ...
            candidate_protocols = set(
                json.loads(candidate.get("network_protocols", "[]"))
            )
            ...
            for tool_protocol in tool_protocols:
                for candidate_protocol in candidate_protocols:
                    if candidate_protocol in compatible_protocols.get(
                        tool_protocol, []
                    ):
                        compatible = True
    ...
    async def find_complementary_matches(
        ...
        for classification in all_classifications:
            ...
            candidate_caps_data = await db.select(
            ...
            if complement_score >= self.match_threshold:
                ...
                tool_data = await db.select_one(
                    "forge_classification", {"id": classification_id}
                )
                secondary_trust = await self._get_trust_score(
                    tool_data["ecosystem"], tool_data["package_name"]
                )
                combined_trust = (primary_trust + secondary_trust) / 2
    ...
    async def find_category_matches(self, classification_id: str) -> list[ToolMatch]:
        ...
        for category in related:
            candidates = await db.select(
                "forge_classification", {"category": category}, limit=50
            )
            ...
            for candidate in candidates:
                ...
                ecosystem_bonus = (
                    0.2 if tool["ecosystem"] != candidate["ecosystem"] else 0.0
                )
    ...
    async def generate_all_matches(self, classification_id: str) -> list[ToolMatch]:
        ...
        all_matches.sort(
            key=lambda m: m.compatibility_score * m.trust_score_combined, reverse=True
        )
    ...
    async def generate_forge_stack(self, use_case: str) -> dict[str, Any]:
        ...
        elif "web" in use_case_lower and (
            "search" in use_case_lower or "research" in use_case_lower
        ):
            stack_config = {
        ...
        return {
            "stack": stack_config,
            "tools": stack_tools,
            "total_trust_score": sum(t["trust_score"] for t in stack_tools)
            / len(stack_tools)
            if stack_tools
            else 0,
