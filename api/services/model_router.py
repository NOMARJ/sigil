...
class ModelRouter:
        ...
        try:
            ...
            if not preview_only:
                await self._log_routing_decision(
                    user_id=user_id, task_type=task_type, routing=routing
                )
            ...
            if not routing["has_sufficient_credits"] and not model_override:
                downgrade = await self._attempt_downgrade(
                    user_balance=user_balance, cost_comparison=cost_comparison
                )
        ...
        try:
            ...
            if potential_opus_cost > 0:
                stats["savings_percentage"] = round(
                    stats["estimated_savings"] / potential_opus_cost * 100, 1
                )
            else:
    ...
    async def _attempt_downgrade(
        self, user_balance: int, cost_comparison: Dict[str, Any]
        ...
        for model in model_preference:
            if model in cost_comparison["alternatives"]:
                ...
                if alt["cost"] <= user_balance:
                    return {"model": model, "cost": alt["cost"]}

        return None

    async def _log_routing_decision(
        self, user_id: str, task_type: str, routing: Dict[str, Any]
    ) -> None:
