...
class ContextAnalyzer:
    ...
    async def analyze_repository_context(
        ...
        repo_fingerprint = self._build_repository_fingerprint(file_contents, static_findings)
        ...
        try:
            ...
            correlation_analysis = self._correlate_findings(static_findings, file_contents)
            ...
            context = {
                ...
                "risk_factors": self._assess_risk_factors(file_contents, static_findings),
    ...
    def _correlate_findings(self, findings: list[Finding], file_contents: dict[str, str]) -> dict[str, Any]:
        ...
        if len(findings_by_file) > 1:
            cross_file_threats = self._identify_cross_file_threats(findings_by_file, file_contents)
        ...
        try:
            ...
            if llm_response.success and llm_response.context_analysis:
                ...
                return {
                    ...
                    "fallback_patterns": self._detect_basic_attack_patterns(file_contents, static_findings),
                }
    ...
    def _assess_risk_factors(self, file_contents: dict[str, str], findings: list[Finding]) -> dict[str, Any]:
        """Assess overall risk factors based on code and findings."""
        
        risk_factors = {
            "high_severity_findings": len([f for f in findings if f.severity.value in ("HIGH", "CRITICAL")]),
            "network_activity": len([f for f in findings if "network" in f.rule or "http" in f.rule]),
            "credential_access": len([f for f in findings if "cred" in f.rule or "password" in f.rule]),
            "code_execution": len([f for f in findings if "eval" in f.rule or "exec" in f.rule]),
            "obfuscation": len([f for f in findings if "obf" in f.rule or "base64" in f.rule]),
            "external_dependencies": self._count_external_dependencies(file_contents),
            "binary_files": len([f for f in file_contents.keys() if self._is_binary_filename(f)]),
        }
    ...
    def _generate_recommendations(
        ...
        if len(architecture["entry_points"]) > 3:
            recommendations.append("Consider reducing the number of entry points to minimize attack surface")
        
        if architecture["complexity_score"] > 8:
            recommendations.append("High code complexity detected - consider refactoring for better security review")
        
        if architecture["suspicious_patterns"]:
            recommendations.append(f"Address {len(architecture['suspicious_patterns'])} suspicious architectural patterns")
        
        # Correlation-based recommendations
        if correlations["cross_file_threats"]:
            recommendations.append("Cross-file threat patterns detected - review coordinated security controls")
        
        if correlations["attack_chains"]:
            recommendations.append("Potential attack chains identified - implement defense in depth")
        
        # Attack pattern-based recommendations
        if attack_patterns.get("attack_chain_detected"):
            recommendations.append("Multi-stage attack pattern detected - review and strengthen all affected components")
        
        if attack_patterns.get("sophistication_level") == "advanced":
            recommendations.append("Advanced threat patterns detected - consider professional security audit")
    ...
    def _build_repository_fingerprint(self, file_contents: dict[str, str], findings: list[Finding]) -> str:
    ...
    def _is_entry_point(self, filename: str, content: str) -> bool:
        ...
        entry_patterns = [
            "main.py", "index.js", "app.py", "__main__.py", "setup.py",
            "main.go", "main.c", "main.cpp", "App.java"
        ]
        ...
        main_patterns = ["def main(", "function main(", "int main(", "if __name__ == '__main__'"]
        return any(pattern in content for pattern in main_patterns)
    
    def _is_config_file(self, filename: str) -> bool:
        """Check if file is a configuration file."""
        config_patterns = [
            ".env", "config.", "settings.", ".ini", ".cfg", ".conf",
    ...
    def _detect_suspicious_architecture_patterns(self, filename: str, content: str) -> list[str]:
    ...
    def _identify_cross_file_threats(
        ...
        cred_files = [f for f, findings in findings_by_file.items() 
        ...
        network_files = [f for f, findings in findings_by_file.items()
                        if any("network" in finding.rule or "http" in finding.rule for finding in findings)]
    ...
    def _cluster_similar_findings(self, findings: list[Finding]) -> dict[str, list[str]]:
    ...
    def _identify_attack_chains(self, findings: list[Finding], file_contents: dict[str, str]) -> list[dict[str, Any]]:
        """Identify potential attack chains."""
        chains = []
    ...
    def _detect_basic_attack_patterns(self, file_contents: dict[str, str], findings: list[Finding]) -> list[str]:
        """Fallback method for basic attack pattern detection."""
        patterns = []
        
        finding_rules = [f.rule for f in findings]
        
        if any("eval" in rule for rule in finding_rules) and any("network" in rule for rule in finding_rules):
            patterns.append("Dynamic code execution with network activity")
        
        if any("cred" in rule for rule in finding_rules) and any("obf" in rule for rule in finding_rules):
