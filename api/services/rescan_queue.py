...
class RescanQueue:
    ...
    async def identify_rescan_candidates(
        self, limit: int = 100
        ...
        try:
            ...
            high_risk_rows = await db.execute_raw_sql(
                high_risk_query, (limit // 2, self.rescan_cooldown_hours)
            )
            ...
            popular_rows = await db.execute_raw_sql(
                popular_query, (limit // 2, self.rescan_cooldown_hours)
            )
    ...
    async def get_queue_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_v1 = sum(
                row["count"] for row in version_stats if row["version"].startswith("1.")
            )
            total_v2 = sum(
                row["count"] for row in version_stats if row["version"].startswith("2.")
            )
            total_packages = total_v1 + total_v2

            migration_percentage = (
                (total_v2 / total_packages * 100) if total_packages > 0 else 0.0
            )
...
async def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show candidates without processing"
    )
    parser.add_argument("--status", action="store_true", help="Show migration status")
    parser.add_argument(
        "--limit", type=int, default=20, help="Number of candidates to show"
    )
    ...
    if args.status:
        ...
        print(
            f"Migration Progress: {status['migration_progress']['percentage_migrated']:.1f}%"
        )
        ...
        print(
            f"  Current hourly rate: {status['recent_activity']['hourly_rate']:.1f}/hour"
        )
        ...
        for version_info in status["version_breakdown"]:
            print(
                f"  {version_info['version']}: {version_info['count']} packages "
    ...
    for i, candidate in enumerate(candidates, 1):
        print(
            f"{i:2d}. {candidate.ecosystem}/{candidate.package_name}@{candidate.package_version}"
        )
        print(
            f"     Current: {candidate.current_verdict} (score: {candidate.current_score:.1f}, "
            f"findings: {candidate.findings_count})"
        )
        print(f"     Priority: {candidate.priority_score:.1f} - {candidate.reason}")
        print(
            f"     Last scanned: {candidate.last_scanned.strftime('%Y-%m-%d %H:%M UTC')}"
        )
        print()
