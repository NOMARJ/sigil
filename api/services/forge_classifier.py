...
class ForgeClassifier:
    ...
    def _extract_scan_patterns(self, findings: list[Finding]) -> dict[str, Any]:
        ...
        for finding in findings:
            ...
            if finding.phase == ScanPhase.CREDENTIALS:
                env_match = re.search(r'(AWS_|GITHUB_|API_KEY|SECRET|TOKEN|PASSWORD|DATABASE_URL|REDIS_URL)', finding.snippet, re.IGNORECASE)
                if env_match:
            ...
            if finding.phase == ScanPhase.CODE_PATTERNS:
                ...
                if any(db in finding.snippet.lower() for db in ["psycopg", "pymongo", "redis", "mysql", "sqlite"]):
                    patterns["import_patterns"].add("database")
                # HTTP clients
                if any(http in finding.snippet.lower() for http in ["requests", "httpx", "axios", "fetch"]):
                    patterns["import_patterns"].add("http_client")
                # File operations
                if any(file_op in finding.snippet.lower() for file_op in ["pathlib", "os.path", "fs.", "file"]):
    ...
    def _build_classification_prompt(self, input_data: ClassificationInput) -> str:
        ...
        findings_summary = []
        for finding in input_data.scan_findings[:10]:  # Limit to first 10 findings to control prompt size
            findings_summary.append(f"- {finding.phase.value}: {finding.description} (severity: {finding.severity.value})")
...
Please respond with valid JSON only:
{
  ...
  "capabilities": [
    ...
    async def classify_package(self, input_data: ClassificationInput) -> ClassificationResult:
        """Classify a single package using Claude Haiku."""
        if not self.api_key:
            logger.warning("No Anthropic API key configured, using rule-based classification")
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                return ClassificationResult(
                    ...
                    description_summary=classification_data.get("description_summary", input_data.description[:500]),
            ...
            logger.error(f"LLM classification failed for {input_data.package_name}: {e}")
            return self._fallback_classification(input_data)
    
    def _fallback_classification(self, input_data: ClassificationInput) -> ClassificationResult:
        ...
        if any(db in description_lower for db in ["postgres", "mysql", "mongo", "redis", "database", "sql"]):
            category = "Database"
        elif any(api in description_lower for api in ["github", "slack", "stripe", "api", "rest", "webhook"]):
            category = "API Integration"
        elif any(code in description_lower for db in ["lint", "test", "format", "compile", "build"]):
            category = "Code Tools"
        elif any(file in description_lower for file in ["file", "filesystem", "git", "directory"]):
            category = "File System"
        elif any(ai in description_lower for ai in ["llm", "ai", "prompt", "embedding", "rag", "gpt", "claude"]):
            category = "AI/LLM"
        elif any(sec in description_lower for sec in ["security", "auth", "secret", "scan", "vulnerability"]):
            category = "Security"
        elif any(dev in description_lower for dev in ["deploy", "ci", "cd", "docker", "kubernetes", "monitoring"]):
            category = "DevOps"
        elif any(comm in description_lower for comm in ["email", "chat", "notification", "message"]):
            category = "Communication"
        elif any(data in description_lower for data in ["etl", "pipeline", "analytics", "transform"]):
            category = "Data Pipeline"
        elif any(search in description_lower for search in ["search", "index", "elastic", "solr"]):
            category = "Search"
        
        # Determine capabilities based on scan findings
        capabilities = []
        if scan_patterns["environment_vars"]:
            capabilities.append({"capability": "requires_env_vars", "confidence": 1.0, "evidence": "Environment variables detected"})
        if scan_patterns["network_protocols"]:
            capabilities.append({"capability": "makes_network_calls", "confidence": 1.0, "evidence": "Network protocols detected"})
        if "database" in scan_patterns["import_patterns"]:
            capabilities.append({"capability": "accesses_database", "confidence": 0.8, "evidence": "Database imports detected"})
        if "file_operations" in scan_patterns["import_patterns"]:
            capabilities.append({"capability": "reads_files", "confidence": 0.8, "evidence": "File operation imports detected"})
    ...
    async def save_classification(self, input_data: ClassificationInput, result: ClassificationResult) -> str:
        """Save classification result to database and return the ID."""
        try:
            # Save main classification record
            classification_record = await db.insert("forge_classification", {
                "ecosystem": input_data.ecosystem,
            ...
            logger.info(f"Saved classification for {input_data.ecosystem}/{input_data.package_name}: {result.category}")
            return classification_id
            
        except Exception as e:
            logger.error(f"Failed to save classification for {input_data.package_name}: {e}")
            raise
    
    async def get_classification(self, ecosystem: str, package_name: str, package_version: str = "") -> dict[str, Any] | None:
    ...
    async def classify_from_public_scan(self, ecosystem: str, package_name: str, package_version: str = "") -> ClassificationResult | None:
