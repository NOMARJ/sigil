...
class AttackChainTracer:
    ...
    async def trace_attack_chain(
        ...
        if not await credit_service.has_credits(user_id, credit_cost):
            raise ValueError(
                f"Insufficient credits. Need {credit_cost} credits for attack chain analysis."
            )
        ...
        try:
            ...
            attack_chain = self._parse_attack_chain_response(
                finding=finding, llm_response=response
            )

            # Store in database if session exists
            if session_id:
                await self._store_attack_chain_analysis(
                    session_id=session_id, attack_chain=attack_chain
                )
    ...
    def _build_attack_chain_prompt(
        self, finding: Finding, context_files: Optional[Dict[str, str]] = None
    ) -> str:
        ...
        if context_files:
            ...
            for filename, content in list(context_files.items())[
                :5
            ]:  # Limit to 5 files
                context += f"\n--- {filename} ---\n{content[:2000]}\n"  # Limit content
...
Include MITRE ATT&CK technique IDs where applicable."""
    ...
    def _parse_attack_chain_response(
        self, finding: Finding, llm_response: Any
    ) -> AttackChain:
        ...
        if llm_response.insights:
            for insight in llm_response.insights:
                ...
                step = AttackStep(
                    ...
                    code_location=insight.affected_files[0]
                    ...
                    mitigation=insight.remediation_suggestions[0]
                ...
                if (
                    "entry" in insight.title.lower()
                    or "input" in insight.description.lower()
                ):
                ...
                for remediation in insight.remediation_suggestions:
                    mitigations.append(
                        {"stage": step.stage.value, "mitigation": remediation}
                    )
        ...
        for i, step in enumerate(attack_steps):
            if step.mitigation:
                kill_chain_disruption[f"Step {i + 1}: {step.stage.value}"] = (
                    step.mitigation
                )
        ...
        return AttackChain(
            ...
            blast_radius={
                "files_affected": len(
                    set(s.code_location for s in attack_steps if s.code_location)
                ),
    ...
    def _build_visualization_data(
        self, attack_steps: List[AttackStep], entry_points: List[str]
    ) -> Dict[str, Any]:
        ...
        for i, entry in enumerate(entry_points):
            nodes.insert(
                0,
                {"id": f"entry_{i}", "label": f"Entry: {entry}", "type": "entry_point"},
            )
    ...
    async def _store_attack_chain_analysis(
        self, session_id: str, attack_chain: AttackChain
    ) -> None:
        ...
        try:
            ...
            await db.execute(
                ...
                """,
                {"session_id": session_id, "analysis": json.dumps(analysis_data)},
            )
