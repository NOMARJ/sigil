...
class SuppressionRule(BaseModel):
    ...
    # Pattern matching
    pattern_type: str = Field(..., description="Type of pattern (SQL_INJECTION, XSS, etc.)")
    ...
    evidence_pattern: Optional[str] = Field(None, description="Evidence pattern (regex)")
    ...
    confidence_adjustment: float = Field(
        ...
        ge=-1.0, 
        le=1.0,
        description="Confidence adjustment (-1 to 1)"
    )
    suppress_completely: bool = Field(False, description="Completely suppress matching findings")
    
    # Metadata
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
    feedback_count: int = Field(1, description="Number of feedbacks supporting this rule")
    last_applied: Optional[datetime] = Field(None, description="Last time rule was applied")
    
    # Learning context
    original_finding_id: str = Field(..., description="ID of finding that triggered rule")
    ...
    effectiveness_score: float = Field(
        ...
        ge=0.0, 
        le=1.0,
        description="Rule effectiveness (0-1)"
    )
...
class FeedbackAggregation(BaseModel):
    ...
    confidence_adjustment: float = Field(0.0, description="Suggested confidence adjustment")
...
class AccuracyMetrics(BaseModel):
    ...
    pattern_accuracy: Dict[str, Dict[str, float]] = Field(
        ...
        description="Accuracy by pattern type"
    )
    ...
    learning_effectiveness: float = Field(
        0.0,
        description="How well the system is learning (0-1)"
    )
