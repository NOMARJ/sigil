...
class ConfidenceSummary(BaseModel):
    ...
    high_confidence_count: int = Field(
        0, description="Number of findings with HIGH confidence"
    )
    medium_confidence_count: int = Field(
        0, description="Number of findings with MEDIUM confidence"
    )
    low_confidence_count: int = Field(
        0, description="Number of findings with LOW confidence"
    )
    average_confidence: float = Field(
        0.0, description="Average confidence score (0.0-1.0)"
    )
    false_positive_likelihood: str = Field(
        "UNKNOWN", description="Estimated false positive rate: LOW/MEDIUM/HIGH/UNKNOWN"
    )
...
class ScanResponseV2(BaseModel):
    ...
    scanner_version: str = Field(
        "2.0.0", description="Version of scanner used for this scan"
    )
    target: str
    target_type: str
    files_scanned: int = 0
    ...
    original_score: Optional[float] = Field(
        None, description="Original score from previous scanner version (if rescanned)"
    )
    rescanned_at: Optional[datetime] = Field(
        None, description="Timestamp when this item was rescanned"
    )
    context_weight: float = Field(
        1.0, description="Context weighting applied to findings"
    )
