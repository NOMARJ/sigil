...
class ToolMetricsCollector:
    ...
    async def collect_metrics_for_tool(self, tool: Dict[str, str]) -> Optional[ToolMetrics]:
        ...
        try:
            ...
            if repo_url and "github.com" in repo_url:
                try:
                    ...
                    self.logger.warning(f"Failed to parse GitHub URL for {tool_id}: {e}")
    ...
    async def store_metrics(self, metrics: ToolMetrics) -> bool:
        ...
        try:
            async with get_database_client() as db:
                ...
                """, 
                metrics.tool_id, metrics.date, metrics.downloads, metrics.stars,
                metrics.version, metrics.forks, metrics.issues_open, 
                metrics.issues_closed, metrics.trust_score)
                
                self.logger.info(f"Stored metrics for {metrics.tool_id}: "
    ...
    async def collect_all_metrics(self) -> None:
        ...
        for tool in tools:
            try:
                ...
                self.logger.error(f"Error processing tool {tool.get('tool_id', 'unknown')}: {e}")
                failed += 1
        
        self.logger.info(f"Metrics collection completed. Success: {successful}, Failed: {failed}")
