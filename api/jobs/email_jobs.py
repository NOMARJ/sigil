...
class EmailJobRunner:
    ...
    async def generate_and_send_weekly_digest(self, test_mode: bool = False) -> None:
        ...
        try:
            ...
            self.logger.info(f"Generating weekly digest for week ending {week_ending.date()}")
            ...
            async with get_database_client() as db:
                ...
                if existing and not test_mode:
                    self.logger.info(f"Weekly digest already sent for {week_ending.date()}")
                    return
            ...
            if content.security_alerts:
                ...
                if alert_count > 0:
                    subject_parts.append(f"🚨 {alert_count} Security Alert{'s' if alert_count > 1 else ''}")
            ...
            campaign_request = EmailCampaignRequest(
                subject=subject,
                content=content,
                send_at=datetime.now() if test_mode else None,  # Send immediately in test mode
                test_mode=test_mode
            )
