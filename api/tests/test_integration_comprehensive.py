...
class TestAuthenticationIntegration:
    ...
    def test_password_validation_integration(self, client: TestClient):
        ...
        for weak_password in weak_passwords:
            ...
            assert resp.status_code == 422, f"Weak password '{weak_password}' was accepted"
...
class TestScanWorkflowIntegration:
    ...
    def test_complete_scan_lifecycle(
        ...
        assert submit_resp.status_code == 201, f"Scan submission failed: {submit_resp.text}"
    ...
    def test_clean_scan_workflow(self, client: TestClient, auth_headers: dict[str, str]):
    ...
    def test_large_scan_integration(self, client: TestClient, auth_headers: dict[str, str]):
...
class TestThreatIntelligenceIntegration:
    ...
    def test_threat_report_to_lookup_integration(
        ...
        report_resp = client.post("/v1/report", json=threat_report_data, headers=auth_headers)
    ...
    def test_signature_management_integration(
        ...
        sig_resp = client.post("/v1/signatures", json=signature_data, headers=auth_headers)
...
class TestBillingIntegration:
    ...
    def test_plan_upgrade_workflow(self, client: TestClient, auth_headers: dict[str, str]):
        ...
        if sub_resp.status_code == 200:
            ...
            subscribe_resp = client.post("/v1/billing/subscribe", json=subscribe_data, headers=auth_headers)
            
            # Should return checkout session URL or handle appropriately
            assert subscribe_resp.status_code in [200, 201, 302]
    
    def test_billing_portal_integration(self, client: TestClient, pro_auth_headers: dict[str, str]):
...
class TestDatabaseIntegration:
    ...
    @pytest.mark.asyncio
    async def test_scan_data_persistence(self, client: TestClient, auth_headers: dict[str, str]):
...
class TestWorkflowIntegration:
    ...
    def test_malware_detection_workflow(self, client: TestClient):
        ...
        scan_resp = client.post("/v1/scans", json=malicious_scan, headers=researcher_headers)
        assert scan_resp.status_code == 201
        ...
        report_resp = client.post("/v1/report", json=threat_data, headers=researcher_headers)
        assert report_resp.status_code == 201
...
class TestErrorHandlingIntegration:
    ...
    def test_database_error_recovery(self, client: TestClient, auth_headers: dict[str, str]):
    ...
    def test_authentication_error_integration(self, client: TestClient):
        ...
        for method, endpoint in protected_endpoints:
            ...
            assert resp.status_code == 401, f"{method} {endpoint} didn't return 401 for invalid token"
    ...
    def test_rate_limit_integration(self, client: TestClient):
        ...
        assert rate_limited or len(responses) == 300, "Rate limiting not properly configured"