...
class TestForgeClassifier:
    ...
    def test_extract_scan_patterns(self, classifier, test_classification_input):
        ...
        patterns = classifier._extract_scan_patterns(test_classification_input.scan_findings)
    ...
    async def test_classification_end_to_end(self, classifier):
        ...
        assert result.category in ["Database", "Uncategorized"]  # Should be Database or fallback
    ...
    async def test_classification_accuracy(self, ground_truth_samples):
        ...
        for sample in ground_truth_samples:
            ...
            if result.category == sample["expected_category"]:
                ...
                print(f"Misclassified {sample['package_name']}: expected {sample['expected_category']}, got {result.category}")
        
        # Calculate accuracy
        accuracy = correct_classifications / total_samples
        print(f"Classification accuracy: {accuracy:.2%} ({correct_classifications}/{total_samples})")
...
class TestForgePerformance:
    ...
    async def test_batch_classification_performance(self):
        ...
        test_inputs = []
        for i in range(100):
            test_inputs.append(ClassificationInput(
                ecosystem="test",
