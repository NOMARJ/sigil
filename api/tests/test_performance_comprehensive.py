...
class TestAPIPerformance:
    ...
    def test_health_endpoint_performance(self, client: TestClient):
        ...
        assert summary["response_times"]["avg"] < 50, f"Health check too slow: {summary['response_times']['avg']:.2f}ms"
        assert summary["response_times"]["p95"] < 100, f"95th percentile too slow: {summary['response_times']['p95']:.2f}ms"
        assert summary["success_rate"] == 100.0, f"Health check failure rate: {100 - summary['success_rate']:.2f}%"
        
        print(f"Health endpoint performance: {summary}")
    
    def test_authentication_performance(self, client: TestClient, test_user_data: dict[str, str]):
        ...
        metrics = measure_endpoint_performance(client, "POST", "/v1/auth/login", login_data, repeat=50)
        summary = metrics.get_summary()
        
        # Authentication should complete within reasonable time
        assert summary["response_times"]["avg"] < 200, f"Login too slow: {summary['response_times']['avg']:.2f}ms"
        assert summary["response_times"]["p95"] < 500, f"Login 95th percentile too slow: {summary['response_times']['p95']:.2f}ms"
        assert summary["success_rate"] >= 98.0, f"Login failure rate too high: {100 - summary['success_rate']:.2f}%"
        
        print(f"Authentication performance: {summary}")
    
    def test_scan_submission_performance(
        self, client: TestClient, auth_headers: dict[str, str], sample_scan_request: dict[str, Any]
    ):
        ...
        assert summary["response_times"]["avg"] < 1000, f"Scan submission too slow: {summary['response_times']['avg']:.2f}ms"
        assert summary["response_times"]["p95"] < 2000, f"Scan 95th percentile too slow: {summary['response_times']['p95']:.2f}ms"
        assert summary["success_rate"] >= 95.0, f"Scan submission failure rate: {100 - summary['success_rate']:.2f}%"
    ...
    def test_threat_lookup_performance(
        ...
        for package_hash in test_hashes:
            metrics = measure_endpoint_performance(
                client, "GET", f"/v1/threat/{package_hash}", headers=auth_headers, repeat=10
            )
            ...
            for code, count in metrics.status_codes.items():
                total_metrics.status_codes[code] = total_metrics.status_codes.get(code, 0) + count
        
        summary = total_metrics.get_summary()
        
        # Threat lookups should be very fast (cached/indexed)
        assert summary["response_times"]["avg"] < 100, f"Threat lookup too slow: {summary['response_times']['avg']:.2f}ms"
        assert summary["response_times"]["p95"] < 200, f"Threat lookup 95th percentile too slow: {summary['response_times']['p95']:.2f}ms"
...
class TestConcurrentLoad:
    ...
    def test_concurrent_health_checks(self, client: TestClient):
        ...
        assert avg_response_time < 100, f"Concurrent health checks too slow: {avg_response_time:.2f}ms"
        assert success_rate >= 95.0, f"Concurrent health check failure rate: {100 - success_rate:.2f}%"
        
        print(f"Concurrent health checks - Avg: {avg_response_time:.2f}ms, Success: {success_rate:.1f}%")
    ...
    def test_concurrent_authentication(self, client: TestClient):
        ...
        assert avg_response_time < 500, f"Concurrent auth too slow: {avg_response_time:.2f}ms"
        assert success_rate >= 90.0, f"Concurrent auth failure rate: {100 - success_rate:.2f}%"
        
        print(f"Concurrent authentication - Avg: {avg_response_time:.2f}ms, Success: {success_rate:.1f}%")
    ...
    def test_concurrent_scan_submission(
        ...
        assert avg_response_time < 2000, f"Concurrent scan submission too slow: {avg_response_time:.2f}ms"
        assert success_rate >= 85.0, f"Concurrent scan failure rate: {100 - success_rate:.2f}%"
        
        print(f"Concurrent scan submission - Avg: {avg_response_time:.2f}ms, Success: {success_rate:.1f}%")
...
class TestDatabasePerformance:
    ...
    async def test_user_lookup_performance(self, client: TestClient):
        ...
        if lookup_times:
            ...
            assert avg_lookup_time < 50, f"User lookup too slow: {avg_lookup_time:.2f}ms"
            assert max_lookup_time < 100, f"Slowest user lookup: {max_lookup_time:.2f}ms"
            
            print(f"User lookup performance - Avg: {avg_lookup_time:.2f}ms, Max: {max_lookup_time:.2f}ms")
    ...
    async def test_scan_query_performance(self, client: TestClient):
        ...
        print(f"Scan query performance - Avg: {avg_query_time:.2f}ms, Max: {max_query_time:.2f}ms")


class TestMemoryUsage:
    """Test memory usage under load."""
    
    def test_memory_usage_during_load(self, client: TestClient, auth_headers: dict[str, str]):
        ...
        print(f"Memory test complete - Initial: {initial_memory:.1f}MB, Final: {final_memory:.1f}MB")
...
class TestClassificationPipelinePerformance:
    ...
    def test_scoring_algorithm_performance(self, sample_findings: list[dict[str, Any]]):
        ...
        start_time = time.time()
        result = scanner.classify_scan_result("large-test", "npm", 1000, large_finding_set)
        end_time = time.time()
        ...
        assert processing_time < 5000, f"Large dataset processing too slow: {processing_time:.2f}ms"


class TestEndpointLatencyUnderLoad:
    """Test endpoint latency under sustained load."""
    
    def test_sustained_load_impact(self, client: TestClient, auth_headers: dict[str, str]):
        """Test how sustained load impacts response times."""
        baseline_metrics = measure_endpoint_performance(client, "GET", "/health", repeat=10)
        ...
        under_load_metrics = measure_endpoint_performance(client, "GET", "/health", repeat=20)
        ...
        assert latency_degradation < 300, f"Latency degraded by {latency_degradation:.1f}% under load"
