...
# ---------------------------------------------------------------------------
# Test-only auth override: validates HS256 JWTs against the in-memory DB.
#
# Installed via app.dependency_overrides in the client fixture so tests can
# authenticate without a real Auth0 connection. The registered_user / pro_user
# fixtures create users directly in db._memory_store and sign tokens with the
# same HS256 secret, bypassing the deprecated /v1/auth/register endpoint.
# ---------------------------------------------------------------------------


async def _test_auth_override(request: Request):
    """Validate a custom HS256 JWT and return the corresponding UserResponse.

    Replaces get_current_user_unified and _compat_current_user_dependency for
    the duration of each test so authenticated endpoints work without Auth0.
    """
    from fastapi import HTTPException, status
    from api.routers.auth import _verify_token
...
def client() -> Iterator[TestClient]:
    ...
    Installs dependency overrides so that get_current_user_unified and
    _compat_current_user_dependency accept the HS256 JWTs generated by the
    registered_user / pro_user fixtures, without requiring a live Auth0 tenant.
    """
    from main import app
    from api.routers.auth import (
        get_current_user_unified,
        _compat_current_user_dependency,
    )

    original_overrides = app.dependency_overrides.copy()
    app.dependency_overrides[get_current_user_unified] = _test_auth_override
    app.dependency_overrides[_compat_current_user_dependency] = _test_auth_override
...
@pytest.fixture()
def registered_user(test_user_data: dict[str, str]) -> dict[str, Any]:
    """Create a test user directly in the in-memory DB and return a
    TokenResponse-shaped dict (access_token, token_type, expires_in, user).

    Bypasses the deprecated POST /v1/auth/register endpoint (410 Gone after
    Auth0 migration) by inserting the user row directly and signing a HS256
    JWT with the same secret the API uses for token validation.
    """
...
@pytest.fixture()
def pro_user(test_user_data: dict[str, str]) -> dict[str, Any]:
    """Create a test user with a PRO subscription directly in the in-memory DB.

    Bypasses the deprecated POST /v1/auth/register endpoint (410 Gone after
    Auth0 migration) using the same direct-insert approach as registered_user.
    """
    from api.routers.auth import _create_access_token
    from api.config import settings
