...
class ForgeUserBehavior(TaskSet):
    ...
    def get_tracked_tools(self):
        ...
        with self.client.get(
            ...
            if (
                ...
                > PERFORMANCE_THRESHOLDS["GET /forge/my-tools"]
            ):
                response.failure(
                    f"Too slow: {response.elapsed.total_seconds() * 1000:.0f}ms"
                )
    ...
    def track_new_tool(self):
        ...
        tool_data["tool_id"] = (
            f"tool-{int(time.time() * 1000)}-{random.randint(1000, 9999)}"
        )
...
def on_request(request_type, name, response_time, response_length, exception, **kwargs):
    ...
    if exception:
        performance_stats["errors"].append(
            {"timestamp": time.time(), "name": name, "exception": str(exception)}
        )
...
def on_test_stop(environment, **kwargs):
    ...
    if isinstance(environment.runner, MasterRunner):
        ...
        successful_requests = sum(
            1 for r in performance_stats["requests"] if r["success"]
        )
        ...
        for endpoint, times in performance_stats["response_times"].items():
            if times:
                ...
                print(
                    f"  P95: {p95:.0f}ms (Threshold: {threshold}ms) {'✓' if passed else '✗'}"
                )
        ...
        for endpoint, times in performance_stats["response_times"].items():
            if times:
                ...
                report["endpoints"][endpoint] = {
                    ...
                    "p99": times_sorted[int(len(times_sorted) * 0.99)],
                    "threshold": PERFORMANCE_THRESHOLDS.get(endpoint, 500),
                    "passed": times_sorted[int(len(times_sorted) * 0.95)]
                    < PERFORMANCE_THRESHOLDS.get(endpoint, 500),
                }
