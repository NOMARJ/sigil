...
class DatabasePerformanceTester:
    ...
    async def analyze_query(self, conn: asyncpg.Connection, query: str, params: List = None) -> Dict:
        ...
        async with self.get_connection() as conn:
            ...
            for query_def in queries:
                ...
                if "error" in result:
                    ...
                    print(f"  Execution Time: {execution_time:.2f}ms (Threshold: {threshold}ms) {'✓' if passed else '✗'}")
    ...
    def analyze_plan(self, plan: Dict) -> List[str]:
        ...
        def check_node(node: Dict, depth: int = 0):
            ...
            if node_type == "Seq Scan":
                ...
                if rows > 1000:
                    issues.append(f"Sequential scan on {node.get('Relation Name', 'unknown')} ({rows} rows)")
    ...
    async def test_under_load(self, conn: asyncpg.Connection):
        ...
        test_queries = [
            ...
            ("SELECT COUNT(*) FROM forge_analytics_events WHERE user_id = $1", ["user-2"]),
        ]
...
async def main():
    ...
    database_url = os.getenv(
        "DATABASE_URL",
        "postgresql://postgres:password@localhost:5432/sigil"
    )
