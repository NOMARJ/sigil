...
class TestForgeToolTracking:
    ...
    def test_track_tool_requires_pro_plan(self, client: TestClient, auth_headers: dict[str, str]):
        ...
        resp = client.post("/v1/forge/tools/track", json=tool_data, headers=free_headers)
        assert resp.status_code == status.HTTP_403_FORBIDDEN
        assert "requires Pro plan" in resp.json()["detail"]

    def test_track_tool_succeeds_with_pro_plan(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        resp = client.post("/v1/forge/tools/track", json=tool_data, headers=pro_auth_headers)
    ...
    def test_list_tracked_tools(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        for tool in tools:
            resp = client.post("/v1/forge/tools/track", json=tool, headers=pro_auth_headers)
            assert resp.status_code == status.HTTP_201_CREATED
    ...
    def test_user_can_only_access_own_tools(self, client: TestClient):
        ...
        for user_data in [user1_data, user2_data]:
            ...
            # Upgrade to Pro
            asyncio.run(db.upsert_subscription(
        ...
        user1_resp = client.post("/v1/auth/login", json={
        ...
        user2_resp = client.post("/v1/auth/login", json={
        ...
        resp = client.post("/v1/forge/tools/track", json=tool_data, headers=user1_headers)
        assert resp.status_code == status.HTTP_201_CREATED
...
class TestForgeAnalytics:
    ...
    def test_personal_analytics_requires_pro_plan(self, client: TestClient, auth_headers: dict[str, str]):
        """Test that personal analytics requires Pro+ plan."""
        resp = client.get("/v1/forge/analytics/personal", headers=auth_headers)
        assert resp.status_code == status.HTTP_403_FORBIDDEN

    def test_personal_analytics_returns_correct_data(self, client: TestClient, pro_auth_headers: dict[str, str]):
    ...
    def test_team_analytics_requires_team_plan(self, client: TestClient, pro_auth_headers: dict[str, str]):
    ...
    def test_analytics_date_filtering(self, client: TestClient, pro_auth_headers: dict[str, str]):
...
class TestForgeSettings:
    ...
    def test_get_forge_settings(self, client: TestClient, pro_auth_headers: dict[str, str]):
    ...
    def test_update_forge_settings(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        resp = client.put("/v1/forge/settings", json=new_settings, headers=pro_auth_headers)
        assert resp.status_code == status.HTTP_200_OK
...
class TestForgeStacks:
    ...
    def test_create_stack_requires_team_plan(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        resp = client.post("/v1/forge/stacks", json=stack_data, headers=pro_auth_headers)
        assert resp.status_code == status.HTTP_403_FORBIDDEN

    def test_list_stacks_requires_team_plan(self, client: TestClient, pro_auth_headers: dict[str, str]):
...
class TestForgeSecurityAndAccess:
    ...
    def test_rate_limiting_by_plan(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        responses = []
        for i in range(5):
            resp = client.post("/v1/forge/tools/track", json={
                **tool_data,
                "name": f"{tool_data['name']} {i}"
            }, headers=pro_auth_headers)
    ...
    def test_cache_invalidation_works(self, client: TestClient, pro_auth_headers: dict[str, str]):
...
class TestForgeIntegration:
    ...
    def test_forge_settings_affect_scan_behavior(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        resp = client.put("/v1/forge/settings", json=settings_data, headers=pro_auth_headers)
        assert resp.status_code == status.HTTP_200_OK
    ...
    def test_tracked_tools_appear_in_main_scan_results(self, client: TestClient, pro_auth_headers: dict[str, str]):
...
class TestForgeEdgeCases:
    ...
    def test_invalid_tool_repository_url(self, client: TestClient, pro_auth_headers: dict[str, str]):
    ...
    def test_duplicate_tool_tracking(self, client: TestClient, pro_auth_headers: dict[str, str]):
    ...
    def test_settings_with_invalid_preferences(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        resp = client.put("/v1/forge/settings", json=invalid_settings, headers=pro_auth_headers)
        assert resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
class TestForgePerformance:
    ...
    def test_large_tool_list_performance(self, client: TestClient, pro_auth_headers: dict[str, str]):
    ...
    def test_analytics_response_time(self, client: TestClient, pro_auth_headers: dict[str, str]):
