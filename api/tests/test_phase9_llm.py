...
class TestLLMAnalysisRequests:
    ...
    @pytest.mark.asyncio 
    async def test_successful_llm_analysis(self, sample_analysis_request: LLMAnalysisRequest):
    ...
    @pytest.mark.asyncio
    async def test_llm_analysis_caching(self, sample_analysis_request: LLMAnalysisRequest):
    ...
    @pytest.mark.asyncio
    async def test_llm_analysis_without_configuration(self, sample_analysis_request: LLMAnalysisRequest):
...
class TestLLMPromptGeneration:
    ...
    @pytest.mark.asyncio
    async def test_analysis_prompt_building(self, sample_analysis_request: LLMAnalysisRequest):
        """Test building analysis prompts for LLM"""
        
        with patch('api.prompts.security_analysis_prompts.SecurityAnalysisPrompts.build_analysis_prompt') as mock_prompt:
    ...
    def test_llm_analysis_types_enumeration(self):
        ...
        assert LLMAnalysisType.PROMPT_INJECTION_DETECTION.value == "prompt_injection_detection"
        ...
        with patch.object(llm_config, 'provider', 'anthropic'):
            with patch.object(llm_config, 'model', 'claude-3-sonnet'):
                with patch('aiohttp.ClientSession.post') as mock_post:
                    ...
                    assert payload["messages"][0]["content"] == "test prompt"
                    assert "response_format" not in payload  # Anthropic doesn't use this
...
class TestPhase9Integration:
    ...
    async def test_phase9_detector_execution(self, phase9_detector):
        ...
        scan_context = {
            "file_contents": {
                "suspicious.py": "eval(base64.b64decode(payload))"
            },
            "static_findings": [
                {
                    "phase": "code_patterns",
...
class TestLLMAnalyticsTracking:
    ...
    async def test_llm_usage_analytics_tracking(self):
        ...
        with patch('api.services.analytics_service.analytics_service.track_llm_usage') as mock_track_llm:
            with patch('api.services.analytics_service.analytics_service.track_threat_discovery') as mock_track_threat:
                with patch.object(llm_service, '_call_llm_api') as mock_call:
