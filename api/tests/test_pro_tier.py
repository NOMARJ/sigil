...
class TestSubscriptionUpgradeFlow:
    ...
    @pytest.mark.asyncio
    async def test_subscription_upgrade_flow(self, client: TestClient, test_user_data: dict[str, str]):
    ...
    async def test_subscription_downgrade_flow(self, client: TestClient, pro_user: dict[str, Any]):
        ...
        # Cancel subscription
        result = await subscription_service.cancel_pro_subscription(user_id, "user_cancelled")
...
class TestLLMDetectionGating:
    ...
    @pytest.mark.asyncio
    async def test_free_user_llm_blocked(self, client: TestClient, auth_headers: dict[str, str]):
    ...
    @pytest.mark.asyncio  
    async def test_pro_user_llm_allowed(self, client: TestClient, pro_auth_headers: dict[str, str]):
        ...
        with patch('api.services.llm_service.LLMService._call_llm_api') as mock_llm:
            ...
            resp = client.post("/v1/scan-enhanced", json=scan_data, headers=pro_auth_headers)
    ...
    async def test_tier_gating_middleware(self):
        ...
        with patch('api.services.subscription_service.subscription_service.get_user_tier') as mock_tier:
...
class TestLLMServiceFailures:
    ...
    @pytest.mark.asyncio
    async def test_llm_service_unavailable_fallback(self, pro_auth_headers: dict[str, str]):
...
class TestProAnalyticsTracking:
    ...
    async def test_llm_usage_tracking(self, pro_auth_headers: dict[str, str]):
        ...
        with patch('api.services.analytics_service.analytics_service.track_llm_usage') as mock_track:
...
class TestProPerformance:
    ...
    async def test_llm_caching_performance(self):
        ...
        analysis_request = LLMAnalysisRequest(
            file_contents={"cached_test.py": "eval('test')"},
            static_findings=[]
        )
