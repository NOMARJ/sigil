...
class TestScanSubmission:
    ...
    def test_submit_scan_returns_scan_id(
        ...
        resp1 = client.post("/v1/scan", json=clean_scan_request, headers=auth_headers)
        resp2 = client.post("/v1/scan", json=clean_scan_request, headers=auth_headers)

        assert resp1.status_code == 200
        assert resp2.status_code == 200
        assert resp1.json()["scan_id"] != resp2.json()["scan_id"]

    def test_submit_scan_critical_findings_high_score(
    ...
    def test_submit_scan_low_risk_verdict(
        self, client: TestClient, auth_headers: dict[str, str]
    ) -> None:
