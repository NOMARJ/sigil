...
class TestPlanGatingSecurity:
    ...
    async def test_plan_downgrade_immediate_effect(self):
        ...
        with patch("api.security.forge_access.get_user_subscription_info") as mock_sub:
            ...
            forge_user = await get_forge_user(user)
            assert has_forge_access(forge_user.subscription_plan, ForgeFeature.TOOL_TRACKING)
        ...
        with patch("api.security.forge_access.get_user_subscription_info") as mock_sub:
            ...
            forge_user = await get_forge_user(user)
            assert not has_forge_access(forge_user.subscription_plan, ForgeFeature.TOOL_TRACKING)
            ...
            with patch("api.database.db.select") as mock_select:
                ...
                for key, value in filters.items():
                    if isinstance(value, str):
                        ...
                        assert "DROP TABLE" not in str(mock_select.call_args) if mock_select.called else True
