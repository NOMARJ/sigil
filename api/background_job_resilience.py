...
class DeadLetterQueue:
    ...
    def list_jobs(
        ...
        jobs.sort(
            key=lambda j: j.metadata.completed_at or datetime.min.replace(tzinfo=timezone.utc),
            reverse=True,
        )
...
class JobRecoveryManager:
    ...
    async def _cleanup_old_data(self):
        ...
        old_completed = [
            job_id for job_id, job in self.job_queue._completed_jobs.items()
...
def background_job(
    ...
    def decorator(func):
        ...
        wrapper.submit_job = lambda *args, **kwargs: job_queue.submit_job(
            func, *args, priority=priority, max_attempts=max_attempts,
