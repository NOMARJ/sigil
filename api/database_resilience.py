...
class ResilientDatabaseManager:
        ...
        try:
            ...
            if self.original_client._pool:
                ...
                if settings.database_configured:
                    ...
                    self.metrics.health_status = DatabaseHealth.DEGRADED  # Using memory store
    ...
    async def get_connection(self) -> AsyncContextManager[Any]:
        ...
        if not self.original_client._pool:
            ...
            if settings.database_configured and self.metrics.health_status == DatabaseHealth.UNAVAILABLE:
                await self._attempt_recovery()
        ...
        try:
            ...
            logger.error("Database connection error (duration=%.2fs): %s", duration, exc)
            error_tracker.track_error(
                DatabaseError(
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if self.metrics.last_successful_query:
            time_since_success = (now - self.metrics.last_successful_query).total_seconds()
        ...
        return {
            ...
            "success_rate": (
                (self.metrics.total_queries - self.metrics.failed_queries) / self.metrics.total_queries * 100
            ...
            "last_successful_query": (
                ...
                if self.metrics.last_successful_query else None
            ),
            "recovery_attempts": self.metrics.recovery_attempts,
            "consecutive_failures": self._connection_failures,
...
class ResilientRedisManager:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recovery_attempts": self.metrics.recovery_attempts,
            "health_status": (
                "healthy" if self._connection_failures < 5 else
                "degraded" if self._connection_failures < 15 else
                "unhealthy"
            ),
        }
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if hasattr(db, '_resilient_manager'):
                    return await db._resilient_manager.execute_with_resilience(func, *args, **kwargs)
                else:
                    return await func(*args, **kwargs)
            except Exception as exc:
