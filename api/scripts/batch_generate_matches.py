...
class BatchMatchGenerator:
    ...
    async def generate_matches_for_tool(self, classification: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Generating matches for {ecosystem}/{package_name} ({category})")
            ...
            existing_matches = await db.select("forge_matches", {
                "primary_classification_id": classification_id
            }, limit=1)
            
            if existing_matches:
                logger.debug(f"Skipping {ecosystem}/{package_name} - matches already exist")
                return True
            ...
            if matches:
                ...
                logger.info(f"✓ Generated {saved_count} matches for {ecosystem}/{package_name}")
            ...
            logger.error(f"Failed to generate matches for {classification.get('package_name', 'unknown')}: {e}")
            return False
    ...
    async def generate_sample_stacks(self):
        ...
        for use_case in sample_use_cases:
            try:
                ...
                logger.info(f"✓ Generated stack: {stack['stack']['name']} ({len(stack['tools'])} tools)")
            except Exception as e:
        ...
        try:
            ...
            for i, classification in enumerate(classifications):
                ...
                if i % 10 == 0:
                    ...
                    eta = (elapsed / (i + 1)) * (total_classifications - i - 1) if i > 0 else 0
                    logger.info(f"Progress: {i}/{total_classifications} ({progress:.1f}%) - ETA: {eta/60:.1f}m")
            ...
            if self.processed_count > 0:
                logger.info(f"Average matches per tool: {self.matches_generated/self.processed_count:.1f}")
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate matches between classified tools")
