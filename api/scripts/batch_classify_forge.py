...
class BatchClassifier:
        ...
        if not self.force:
            ...
            for package in packages:
                ...
                if not existing:
                    ...
                    logger.debug(f"Skipping {package['ecosystem']}/{package['package_name']} - already classified")
    ...
    async def classify_package(self, package: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Classifying {ecosystem}/{package_name} ({len(findings)} findings)")
            ...
            logger.error(f"Failed to classify {package.get('package_name', 'unknown')}: {e}")
            return False
        ...
        try:
            ...
            for i, package in enumerate(packages):
                ...
                if i % 10 == 0:
                    ...
                    logger.info(f"Progress: {i}/{total_packages} ({progress:.1f}%) - ETA: {eta/60:.1f}m")
            ...
            if self.classified_count > 0:
                logger.info(f"Average time per classification: {elapsed/self.classified_count:.1f} seconds")
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Batch classify packages for Sigil Forge")
    parser.add_argument("--ecosystem", help="Filter by ecosystem (clawhub, mcp, npm, pypi)")
    ...
    parser.add_argument("--force", action="store_true", help="Re-classify existing classifications")
    ...
    if not settings.anthropic_api_key and not args.dry_run:
        logger.warning("No Anthropic API key configured. Will use rule-based classification.")
