...
class MCPWatchdogScheduler:
    ...
    async def _discovery_scheduler(self, config: dict):
        ...
        while self.running:
            try:
                ...
                await self._update_config({"last_discovery_run": datetime.now(timezone.utc).isoformat()})
                
                logger.info(f"✅ Discovery scan complete. Next scan in {interval_hours} hours.")
    ...
    async def _run_discovery(self):
        ...
        from api.services.mcp_crawler import discover_mcp_servers, store_mcp_results, scan_mcp_server
    ...
    async def _run_typosquat_check(self):
        ...
        from api.services.mcp_crawler import detect_typosquats, send_typosquat_alerts, store_typosquat_alerts
        from api.database import db
...
async def show_status():
    ...
    try:
        ...
        total_alerts = await db.select("typosquat_alerts", filters={"ecosystem": "mcp"}, limit=1000)
        ...
        if config:
            ...
            print(f"Scan Interval: {config.get('scan_interval_hours', 'unknown')} hours")
            print(f"Last Discovery: {config.get('last_discovery_run', 'never')}")
            print(f"Last Typosquat Check: {config.get('last_typosquat_check', 'never')}")
        else:
