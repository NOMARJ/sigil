...
class PerformanceMonitor:
    ...
    async def check_index_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Analyze results
            active_indexes = [
                idx for idx in index_stats if idx["usage_status"] == "ACTIVE"
            ]
            unused_indexes = [
                idx for idx in index_stats if idx["usage_status"] == "UNUSED"
            ]
            ...
            for idx in top_indexes:
                total_ops = (
                    (idx["user_seeks"] or 0)
                    + (idx["user_scans"] or 0)
                    + (idx["user_lookups"] or 0)
                )
                print(
                    f"  📊 {idx['table_name']}.{idx['index_name']}: {total_ops:,} operations"
                )
            ...
            if unused_indexes:
                ...
                for idx in unused_indexes:
                    if idx["index_name"] and not idx["index_name"].startswith("PK_"):
                        print(
                            f"  🗑️  {idx['table_name']}.{idx['index_name']} ({idx['index_type']})"
                        )
    ...
    async def check_index_fragmentation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            needs_rebuild = [
                idx for idx in frag_stats if idx["recommendation"] == "REBUILD REQUIRED"
            ]
            needs_reorg = [
                idx
                for idx in frag_stats
                if idx["recommendation"] == "REORGANIZE RECOMMENDED"
            ]

            print(
                f"🔧 {len(needs_rebuild)} indexes need rebuilding (>30% fragmentation)"
            )
            print(
                f"🔄 {len(needs_reorg)} indexes need reorganizing (10-30% fragmentation)"
            )
            ...
            if needs_rebuild:
                ...
                for idx in needs_rebuild:
                    print(
                        f"  🔨 {idx['table_name']}.{idx['index_name']}: {idx['avg_fragmentation_in_percent']:.1f}% fragmented"
                    )
    ...
    async def analyze_slow_queries(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_queries = [
                q for q in slow_queries if q["avg_elapsed_time_ms"] > 1000
            ]  # > 1 second
            slow_queries_filtered = [
                q for q in slow_queries if q["avg_elapsed_time_ms"] > 200
            ]  # > 200ms
            ...
            if critical_queries:
                ...
                for i, query in enumerate(critical_queries[:5], 1):
                    ...
                    print(
                        f"  {i}. {query['avg_elapsed_time_ms']:.0f}ms avg, {query['execution_count']} executions"
                    )
    ...
    async def benchmark_common_operations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"  📊 Forge classification search: {duration_ms:.1f}ms ({len(results)} results)"
            )
        ...
        try:
            ...
            print(
                f"  📊 Public scans filter: {duration_ms:.1f}ms ({len(results)} results)"
            )
        ...
        try:
            ...
            print(
                f"  📊 Complex JOIN query: {duration_ms:.1f}ms ({len(results)} results)"
            )
        ...
        try:
            start_time = time.time()
            results = await db.select(
                "scans", {}, limit=20, order_by="created_at", order_desc=True
            )
            end_time = time.time()
            duration_ms = (end_time - start_time) * 1000
        ...
        if db.connected:
            ...
            try:
                ...
                if hasattr(db._pool, "size"):
                    ...
                    print(
                        f"📊 Connection pool: {health['active_connections']}/{health['pool_size']} active"
                    )
    ...
    async def generate_optimization_recommendations(self) -> List[str]:
        ...
        if not recommendations:
            recommendations.append(
                "Database performance looks good! Consider regular maintenance."
            )
        else:
    ...
    async def run_full_analysis(self) -> Dict[str, Any]:
        ...
        if (
            "slow_queries" in self.results
            and "error" not in self.results["slow_queries"]
        ):
            print(
                f"Critical Slow Queries: {self.results['slow_queries']['critical_queries']}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--check-indexes",
        action="store_true",
        help="Check index usage and fragmentation",
    )
    parser.add_argument(
        "--analyze-queries", action="store_true", help="Analyze slow queries"
    )
    parser.add_argument(
        "--benchmark", action="store_true", help="Run performance benchmarks"
    )
    parser.add_argument(
        "--health-check", action="store_true", help="Check connection health"
    )
    ...
    try:
        ...
        if args.all:
            ...
            if args.check_indexes:
                ...
                monitor.results[
                    "fragmentation"
                ] = await monitor.check_index_fragmentation()
            ...
            if args.benchmark:
                monitor.results[
                    "benchmarks"
                ] = await monitor.benchmark_common_operations()
