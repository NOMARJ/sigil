...
async def demo_matching(classifications):
    ...
    postgres_tools = [
        (input_data, result) for input_data, result in classifications
    ...
    for use_case in use_cases:
        ...
        try:
            ...
            for tool in stack['tools']:
                print(f"    - {tool['install_command']} (trust: {tool['trust_score']:.0f})")
...
async def demo_api_formats():
    ...
    sample_classification = {
        ...
        "capabilities": [
            {"capability": "accesses_database", "confidence": 0.9, "evidence": "PostgreSQL driver detected"},
            {"capability": "requires_env_vars", "confidence": 1.0, "evidence": "DATABASE_URL required"}
        ],
        "trust_score": 85.0,
    ...
    agent_format = {
        ...
        "capabilities": [cap["capability"] for cap in sample_classification["capabilities"]],
        "environment_vars": sample_classification["environment_vars"],
        "verdict": "LOW_RISK" if sample_classification["trust_score"] >= 80 else "MEDIUM_RISK"
    }
