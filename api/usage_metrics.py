...
class UsageEvent(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Type of event (llm_scan, threat_detected, etc.)"
    )
    event_data: Optional[Dict[str, Any]] = Field(
        None, description="Event-specific data as JSON"
    )
...
class LLMUsageRecord(BaseModel):
    ...
    processing_time_ms: int = Field(
        0, description="Time taken for analysis in milliseconds"
    )
    ...
    threats_found: int = Field(
        0, description="Number of threats detected (confidence > 0.7)"
    )
    ...
    fallback_used: bool = Field(
        False, description="Whether fallback was used due to errors"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class ThreatDiscovery(BaseModel):
    ...
    threat_type: str = Field(
        ..., description="Category of threat (code_injection, credential_theft, etc.)"
    )
    severity: str = Field(
    ...
    threat_hash: Optional[str] = Field(
        None, description="SHA-256 hash for deduplication"
    )
    analysis_type: str = Field(
        "llm_analysis", description="Type of analysis that found this threat"
    )
    evidence_snippet: Optional[str] = Field(
        None, description="Code snippet showing the threat"
    )
    remediation_suggested: List[str] = Field(
        default_factory=list, description="Suggested remediation steps"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class UserEngagementMetrics(BaseModel):
    ...
    session_duration_minutes: int = Field(
        0, description="Total session time in minutes"
    )
    features_used: List[str] = Field(
        default_factory=list, description="Features accessed during the day"
    )
    upgrade_prompts_shown: int = Field(
        0, description="Number of upgrade prompts shown to free users"
    )
    upgrade_prompts_dismissed: int = Field(
        0, description="Number of upgrade prompts dismissed"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class BillingUsageSummary(BaseModel):
    ...
    total_cost_cents: Decimal = Field(
        Decimal("0.00"), description="Total cost in cents"
    )
    total_threats_found: int = Field(0, description="Total threats discovered")
    zero_days_discovered: int = Field(
        0, description="Zero-day discoveries in this period"
    )
    overage_charges_cents: Decimal = Field(
        Decimal("0.00"), description="Charges for usage beyond plan limits"
    )
...
class ChurnRiskMetrics(BaseModel):
    ...
    risk_score: int = Field(
        ..., description="Risk score (0-100, higher = more likely to churn)"
    )
    risk_category: str = Field(
        ...,
        description="Risk category (HEALTHY, LOW_ENGAGEMENT, MEDIUM_RISK, HIGH_RISK)",
    )
    last_scan_date: Optional[datetime] = Field(
        None, description="Date of last scan activity"
    )
    monthly_scans: int = Field(0, description="Number of scans in last 30 days")
    threat_hit_rate: float = Field(
        0.0, description="Percentage of scans that found threats"
    )
    avg_session_duration: float = Field(
        0.0, description="Average session duration in minutes"
    )
    feature_adoption_score: float = Field(
        0.0, description="Score based on features used (0-1)"
    )
    upgrade_prompt_response: Optional[str] = Field(
        None, description="Response to upgrade prompts (for free users)"
    )
...
class ThreatTrendAnalysis(BaseModel):
    ...
    unique_users_affected: int = Field(
        0, description="Number of unique users who encountered this threat"
    )
...
class UserUsageStats(BaseModel):
    ...
    cost_this_period: Decimal = Field(
        Decimal("0.00"), description="Estimated cost this period"
    )
    ...
    plan_limits: Dict[str, Any] = Field(
        default_factory=dict, description="Current plan limits"
    )
    usage_by_day: List[Dict[str, Any]] = Field(
        default_factory=list, description="Daily breakdown"
    )
    top_threat_categories: List[Dict[str, Any]] = Field(
        default_factory=list, description="Most common threat types found"
    )
...
class AnalyticsQuery(BaseModel):
    ...
    tiers: Optional[List[str]] = Field(
        None, description="Filter to specific subscription tiers"
    )
...
class UsageAnalyticsQuery(AnalyticsQuery):
    ...
    include_cost_breakdown: bool = Field(
        False, description="Include detailed cost analysis"
    )
    include_model_performance: bool = Field(
        False, description="Include model performance metrics"
    )
...
class ThreatAnalyticsQuery(AnalyticsQuery):
    ...
    include_zero_days_only: bool = Field(
        False, description="Only include zero-day discoveries"
    )
    severity_levels: Optional[List[str]] = Field(
        None, description="Filter to specific severity levels"
    )
...
class ChurnAnalyticsQuery(BaseModel):
    ...
    risk_categories: Optional[List[str]] = Field(
        None, description="Filter to specific risk categories"
    )
    min_risk_score: int = Field(0, description="Minimum risk score threshold")
    include_recommendations: bool = Field(
        True, description="Include retention recommendations"
    )
...
class BusinessMetricsSummary(BaseModel):
    ...
    # Revenue metrics
    active_pro_subscribers: int = Field(
        0, description="Number of active Pro subscribers"
    )
    mrr_cents: Decimal = Field(
        Decimal("0.00"), description="Monthly recurring revenue in cents"
    )
    ...
    # Cost metrics
    llm_costs_cents: Decimal = Field(
        Decimal("0.00"), description="LLM API costs in cents"
    )
    cost_per_insight: Decimal = Field(
        Decimal("0.00"), description="Cost per AI insight generated"
    )
    margin_percentage: float = Field(0.0, description="Gross margin percentage")

    # Quality metrics
    avg_threat_confidence: float = Field(
        0.0, description="Average confidence of threat detections"
    )
    ...
    user_satisfaction_score: Optional[float] = Field(
        None, description="User satisfaction score if available"
    )
...
class AnalyticsConfig(BaseModel):
    ...
    track_individual_events: bool = Field(
        True, description="Track individual usage events"
    )
    aggregate_daily_metrics: bool = Field(
        True, description="Create daily metric aggregations"
    )
    retention_days: int = Field(
        90, description="Days to retain detailed analytics data"
    )
    cost_calculation_enabled: bool = Field(
        True, description="Calculate LLM usage costs"
    )
    churn_prediction_enabled: bool = Field(
        True, description="Enable churn risk prediction"
    )
    real_time_alerts: bool = Field(False, description="Enable real-time usage alerts")
    anonymize_after_days: int = Field(
        365, description="Days after which to anonymize user data"
    )
