...
class ScannerEngine:
    ...
    async def scan_with_pro_features(
        ...
        if user_tier in ("PRO", "TEAM", "ENTERPRISE"):
            try:
                ...
                if file_contents:
                    ...
                    logger.info(
                        f"LLM analysis completed: {len(llm_findings)} additional findings"
                    )
                else:
        ...
        logger.info(
            f"Total scan completed: {len(all_findings)} findings across all phases"
        )
        return all_findings

    async def _collect_file_contents(
        self, root_path: Path, max_files: int = 50
    ) -> dict[str, str]:
        """
        Collect file contents for LLM analysis.
        ...
        try:
            for file_path in self._walk_files(root_path):
                if files_collected >= max_files:
                    logger.info(
                        f"Reached max file limit ({max_files}) for LLM analysis"
                    )
                    break

                # Only include text files under reasonable size
                if (
                    file_path.suffix in text_extensions
                    and file_path.stat().st_size < 100_000
                ...
                # Also include package.json, setup.py, requirements.txt etc. even if no extension
                    ...
                    and file_path.stat().st_size < 50_000
                ):
                    try:
                        content = file_path.read_text(errors="replace")
                        relative_path = str(file_path.relative_to(root_path))
