...
class Phase9LLMDetector:
    ...
    async def scan_with_llm(
        ...
        try:
            ...
            if not analysis_response.success:
                logger.warning(f"LLM analysis failed: {analysis_response.error_message}")
    ...
    def _convert_insights_to_findings(self, analysis_response) -> list[Finding]:
        ...
        for insight in analysis_response.insights:
            ...
            severity = self._map_threat_to_severity(insight.threat_category, insight.confidence)
            ...
            primary_file = insight.affected_files[0] if insight.affected_files else "<multiple>"
    ...
    def _map_threat_to_severity(self, threat_category: LLMThreatCategory, confidence: float) -> Severity:
    ...
    def _build_llm_explanation(self, insight) -> str:
        ...
        if insight.false_positive_likelihood > 0.3:
            parts.append(f"⚠️ False Positive Risk: {insight.false_positive_likelihood:.1%}")
