...
async def get_user_subscription_info(user_id: str) -> Dict[str, Any]:
    ...
    team_plan = (
        SigilPlan(user_data.get("team_plan", "free").lower())
    ...
    effective_plan = (
        team_plan
        if plan_hierarchy.index(team_plan) > plan_hierarchy.index(user_plan)
        else user_plan
    )
    ...
    return {
        ...
        "team_role": TeamRole(user_data.get("team_role", "member"))
        if user_data.get("team_id")
        else None,
...
async def get_forge_user(
    ...
    usage_key = (
        f"api_usage:{current_user.id}:{datetime.utcnow().strftime('%Y-%m-%d-%H')}"
    )
        ...
        if int(current) >= rate_limit:
            raise HTTPException(
                ...
                detail={
                    ...
                    "period": "hour",
                    "retry_after": 3600
                    - (datetime.utcnow().minute * 60 + datetime.utcnow().second),
...
class DataAccessFilter:
    ...
    async def filter_results(
        results: List[Dict], forge_user: ForgeUser, scope: str = "user"
    ) -> List[Dict]:
        ...
        if scope == "user":
            ...
            return [
                r
                for r in results
                if r.get("organization_id") == forge_user.organization_id
            ]
