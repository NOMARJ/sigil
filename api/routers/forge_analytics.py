...
from api.models import (
    PlanTier, ForgeEventType, PersonalAnalyticsRequest, PersonalAnalyticsResponse,
    TeamAnalyticsResponse, OrganizationAnalyticsResponse,
    AnalyticsEventCreateRequest, AnalyticsEventBatchRequest
)
...
async def track_events_batch(
    ...
    if len(request.events) > 100:
        raise HTTPException(
            status_code=400,
            detail="Maximum 100 events per batch request"
        )
...
async def export_personal_analytics(
    ...
    await track_forge_event(
        ...
        event_data={"export_type": "personal_analytics", "days_back": request.days_back}
    )
...
async def get_team_member_analytics(
    ...
    if current_user.team_id != team_id:
        raise HTTPException(
            status_code=403,
            detail="Access denied to this team's analytics"
        )
...
async def get_organization_analytics(
    ...
    try:
        ...
        await track_forge_event(
            user_id=current_user.id,
            event_type=ForgeEventType.ANALYTICS_VIEWED,
            event_data={"analytics_type": "organization", "organization_id": organization_id}
        )
...
async def get_realtime_dashboard(
    ...
    dashboard_data = {
        ...
        "features_available": {
            "personal_analytics": user_plan in [PlanTier.PRO, PlanTier.TEAM, PlanTier.ENTERPRISE],
    ...
    await track_forge_event(
        ...
        event_data={"plan": user_plan.value, "timestamp": datetime.utcnow().isoformat()}
    )
...
async def get_analytics_config(
    ...
    config = {
        ...
        } if user_plan != PlanTier.ENTERPRISE else None
    }
