...
from api.services.realtime_dashboard import dashboard_service, send_security_notification
...
async def websocket_dashboard(
    ...
    logger.info(f"WebSocket connection attempt: user={user_id}, subs={subscription_list}")
...
async def invalidate_caches(
    ...
    if invalid_types:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid cache types: {invalid_types}"
        )
