...
async def rescan_package(
    ...
    try:
        ...
        logger.info("Rescanning %s with Scanner v2 (current: %s, score: %.1f)", 
                   scan_id, current_verdict, current_score)
        ...
        score_change_percentage = 0.0
        if current_score > 0:
            score_change_percentage = ((current_score - new_score) / current_score) * 100
        ...
        logger.info(
            ...
            scan_id, current_verdict, current_score, new_verdict.value, 
            new_score, score_change_percentage
        )
...
@router.post(
    ...
    dependencies=[Depends(RateLimiter(max_requests=5, window=300))],  # Stricter rate limit for batches
)
...
def _apply_context_filtering(finding: Finding, scan_record: Dict[str, Any]) -> Finding | None:
    """
    Apply context-aware filtering to reduce false positives.
    ...
    if any(pattern in file_path for pattern in ["/docs/", "/test/", "/spec/", "readme", ".md"]):
