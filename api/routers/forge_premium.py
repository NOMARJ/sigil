...
async def track_tool(
    ...
    try:
        ...
        existing = await db.select_one("forge_user_tools", {
        ...
        if existing:
            raise HTTPException(
                status_code=409,
                detail="Tool is already being tracked"
            )
...
async def untrack_tool(
    ...
    try:
        ...
        tracking = await db.select_one("forge_user_tools", {
            "user_id": current_user.id,
            "tool_id": tool_id,
            "ecosystem": ecosystem
        })
        ...
        if not tracking:
            raise HTTPException(
                status_code=404,
                detail="Tool is not being tracked"
            )
...
async def update_stack(
    ...
    try:
        ...
        if stack["user_id"] != current_user.id:
            raise HTTPException(
                status_code=403,
                detail="You can only update your own stacks"
            )
...
async def delete_stack(
    ...
    try:
        ...
        if stack["user_id"] != current_user.id:
            raise HTTPException(
                status_code=403,
                detail="You can only delete your own stacks"
            )
        ...
        await _track_analytics_event(
            ...
            {
                "stack_id": stack_id,
                "was_public": stack.get("is_public", False)
            }
        )
...
async def get_alert_subscriptions(
    ...
    try:
        ...
        for sub in subscriptions:
            results.append(AlertSubscription(
                id=str(sub["id"]),
...
async def create_alert_subscription(
    ...
    try:
        ...
        logger.error(f"Create alert subscription failed for user {current_user.id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_alert_subscription(
    ...
    try:
        # Find subscription
        subscription = await db.select_one("forge_alert_subscriptions", {
            "id": subscription_id,
            "user_id": current_user.id
        })
        ...
        if updates:
            await db.update("forge_alert_subscriptions", {"id": subscription_id}, updates)
        
        # Get updated subscription
        updated = await db.select_one("forge_alert_subscriptions", {"id": subscription_id})
        ...
        logger.error(f"Update alert subscription failed for user {current_user.id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_alert_subscription(
    ...
    try:
        ...
        logger.error(f"Delete alert subscription failed for user {current_user.id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_forge_settings(
    ...
    try:
        settings = await db.select_one("forge_user_settings", {
            "user_id": current_user.id
        })
        ...
        return ForgeUserSettings(
            ...
            alert_types=json.loads(settings.get("alert_types", '["security", "updates"]')),
            delivery_channels=json.loads(settings.get("delivery_channels", '["email"]')),
            quiet_hours=json.loads(settings["quiet_hours"]) if settings.get("quiet_hours") else None,
...
async def update_forge_settings(
    ...
    try:
        ...
        existing = await db.select_one("forge_user_settings", {
            "user_id": current_user.id
        })
        
        if existing:
            await db.update("forge_user_settings", {"user_id": current_user.id}, updates)
        else:
            ...
            new_settings = {
                ...
                "alert_types": json.dumps(request.alert_types or ["security", "updates"]),
                "delivery_channels": json.dumps(request.delivery_channels or ["email"]),
                "quiet_hours": json.dumps(request.quiet_hours) if request.quiet_hours else None,
                "email_notifications": request.email_notifications if request.email_notifications is not None else True,
                "slack_notifications": request.slack_notifications if request.slack_notifications is not None else False,
                "weekly_digest": request.weekly_digest if request.weekly_digest is not None else True,
                "created_at": now,
                "updated_at": now
            }
        ...
        updated = await db.select_one("forge_user_settings", {
            "user_id": current_user.id
        })
        ...
        return ForgeUserSettings(
            ...
            alert_types=json.loads(updated.get("alert_types", '["security", "updates"]')),
            delivery_channels=json.loads(updated.get("delivery_channels", '["email"]')),
            quiet_hours=json.loads(updated["quiet_hours"]) if updated.get("quiet_hours") else None,
