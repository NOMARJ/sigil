...
def extract_permissions_from_scan(scan_data: dict) -> dict[str, list[str]]:
    ...
    for finding in findings:
        ...
        # File system access
        if any(keyword in rule for keyword in ["file", "path", "directory", "read", "write"]):
            file_patterns = [
                r"([~/]\S+\.\w+)",
                r"(/[a-zA-Z0-9/_.-]+)",
                r"(\.\/[a-zA-Z0-9/_.-]+)"
        ...
        # Database patterns
        db_indicators = ["database", "sql", "postgres", "mysql", "mongodb", "redis", "sqlite"]
        if any(indicator in rule or indicator in snippet for indicator in db_indicators):
...
@router.get("/permissions", response_class=HTMLResponse, summary="MCP Permissions Directory")
async def permissions_directory() -> HTMLResponse:
    ...
    try:
        ...
        return HTMLResponse(content=f"<html><body><h1>Error</h1><p>Failed to load permissions: {e}</p></body></html>")

@router.get("/permissions/{mcp_name}", response_class=HTMLResponse, summary="Individual MCP server permissions")
async def mcp_permissions_page(
    mcp_name: str = Path(..., min_length=1, max_length=200, regex=r"^[a-zA-Z0-9_\-/]+$")
) -> HTMLResponse:
    ...
    try:
        ...
        scan = await db.select_one(
            "public_scans",
            {"ecosystem": "mcp", "package_name": mcp_name}
        )
...
async def search_permissions(
    permission: str | None = Query(
        None, description="Permission type (environment, filesystem, network, etc.)",
    ...
    try:
        ...
        for scan in scans:
            ...
            results.append({
                "name": scan["package_name"],
                "author": metadata.get("author", "unknown"),
                "description": metadata.get("description", "")[:100] + ("..." if len(metadata.get("description", "")) > 100 else ""),
                "stars": metadata.get("stars", 0),
