...
@router.get(
    ...
    dependencies=[Depends(RateLimiter(max_requests=30, window=300))],  # 30 requests per 5 minutes
)
async def get_scanner_metrics(
    current_user: Annotated[UserResponse, Depends(get_current_user_unified)],
    days: int = Query(30, ge=1, le=90, description="Number of days to include in analysis"),
    ...
    """
    try:
        logger.info("Collecting scanner metrics for %d days (user: %s)", days, current_user.id)
        ...
        if not include_details:
            ...
            return {
                "period_days": days,
                "migration_percentage": migration_progress.get("migration_percentage", 0),
                "false_positive_improvement": improvements.get("false_positive_reduction_percentage", 0),
        ...
        logger.exception("Failed to get scanner metrics for user %s: %s", current_user.id, e)
        return {
            "error": str(e),
            "period_days": days,
...
@router.get(
    ...
    dependencies=[Depends(RateLimiter(max_requests=60, window=300))],  # Higher limit for dashboard polling
)
async def get_migration_progress(
    ...
    try:
        ...
        return {
            ...
            "recent_activity": {
                "rescanned_last_24h": rescan_stats.get("recent_activity", {}).get("rescanned_last_24h", 0),
                "rescanned_last_7_days": rescan_stats.get("recent_activity", {}).get("rescanned_last_7_days", 0),
            },
            "improvements": {
                "false_positive_reduction": improvements.get("false_positive_reduction_percentage", 0),
        ...
        logger.exception("Failed to get migration progress for user %s: %s", current_user.id, e)
        return {
            "migration": {"percentage": 0, "v1_scans": 0, "v2_scans": 0, "total_scans": 0},
            "recent_activity": {"rescanned_last_24h": 0, "rescanned_last_7_days": 0},
            "improvements": {"false_positive_reduction": 0, "score_reduction": 0, "status": False},
            "error": str(e),
        }
...
async def get_false_positive_analysis(
    ...
    try:
        ...
        return {
            ...
            "version_breakdown": {
                "v1_verdicts": version_comparison.get("v1", {}).get("verdict_distribution", {}),
                "v2_verdicts": version_comparison.get("v2", {}).get("verdict_distribution", {}),
            },
            "validation": {
        ...
        logger.exception("Failed to get false positive analysis for user %s: %s", current_user.id, e)
        return {
            "period_days": days,
            "false_positive_rates": {"v1_estimated": None, "v2_estimated": None, "improvement": None},
            "confidence_analysis": {"total_low_confidence": 0, "total_high_confidence": 0},
