...
class RescanWorker:
        ...
        try:
            ...
            if existing_record:
                ...
                logger.info(
                    ...
                    new_verdict.value,
                    new_score,
                    ((original_score - new_score) / original_score * 100) if original_score > 0 else 0
                )
    ...
    async def process_queue_batch(self, batch_size: int = 5) -> int:
        ...
        logger.info("Batch complete: %d/%d packages rescanned successfully", successful, len(candidates))
        return successful
    ...
    async def run_continuous(self, check_interval: int = 300):
        ...
        """
        logger.info("Starting continuous rescan worker (check interval: %d seconds)", check_interval)
        self.running = True
