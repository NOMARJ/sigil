...
- **A keyword argument that repeats a bound name is not a link.** Every
  ...
  clean skills or on 326 MCP servers. In a Python file a bare name inside
  `{...}` is still a value (`json={token: ...}`, `f"{token:>40}"`): Python
  evaluates it, where JavaScript only names a property. Braces in a string's
  text (`"{token:>40}"`, a `.format` template, `f"{{token}}"`) are not code.
