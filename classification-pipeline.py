...
FORGE_CATEGORIES = {
    ...
    "devops": {
        "name": "DevOps Tools",
        "keywords": ["docker", "kubernetes", "deploy", "ci", "cd", "terraform", "ansible"],
...
class ForgeClassifier:
    ...
    def _rule_based_classify(self, input_data: ClassificationInput) -> ClassificationResult:
        ...
        for category_id, category_def in FORGE_CATEGORIES.items():
            ...
            keyword_matches = sum(1 for kw in category_def["keywords"] 
                                if kw in description_lower)
            score += keyword_matches * 0.3
            
            # Score based on environment variables
            env_matches = sum(1 for pattern in category_def["env_patterns"]
                            for env_var in input_data.env_vars_detected
                            if pattern in env_var)
            score += env_matches * 0.4
            
            # Score based on imports
            import_matches = sum(1 for pattern in category_def["import_patterns"]
        ...
        return ClassificationResult(
            ...
            secondary_categories=[cat for cat, score in category_scores.items() 
                                if score > 0.5 and cat != primary_category][:2],
    ...
    async def _llm_classify(self, input_data: ClassificationInput, 
                          rule_hint: ClassificationResult) -> ClassificationResult:
    ...
    def _build_classification_prompt(self, input_data: ClassificationInput, 
                                   rule_hint: ClassificationResult) -> str:
        """Build classification prompt for Claude."""
        
        categories_list = "\n".join([f"- {cat_id}: {cat_def['name']}" 
...
{{
    ...
    def _parse_llm_response(self, response: str, input_data: ClassificationInput) -> ClassificationResult:
    ...
    def _extract_runtime(self, input_data: ClassificationInput) -> List[str]:
        ...
        if input_data.ecosystem in ["skills", "mcp"]:
            ...
            if any(imp in ["requests", "fastapi", "flask", "django"] 
                   for imp in input_data.imports_detected):
                runtimes.append("python")
            if any(imp in ["express", "axios", "node-fetch"] 
                   for imp in input_data.imports_detected):
...
class BatchClassificationJob:
    ...
    async def run_classification_batch(self, batch_size: int = 50, max_packages: Optional[int] = None):
        ...
        for batch in self._chunked(unclassified, batch_size):
            ...
            for package_data in batch:
                try:
                    ...
                    logger.info(f"Classified {input_data.package_name}: {result.primary_category} "
                              f"(confidence: {result.confidence_score:.2f})")
                              
                except Exception as e:
                    logger.error(f"Classification failed for {package_data.get('package_name')}: {e}")
                    error_count += 1
        ...
        logger.info(f"Batch classification complete: {classified_count} classified, {error_count} errors")
        
        # Trigger stack generation after classification
        await self._trigger_stack_generation()
        
    async def _get_unclassified_packages(self, batch_size: int, max_packages: Optional[int]) -> List[Dict]:
    ...
    async def _save_classification_batch(self, batch_results: List[Tuple[Dict, ClassificationResult]]):
