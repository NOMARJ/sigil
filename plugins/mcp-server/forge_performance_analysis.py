...
class ForgePerformanceAnalyzer:
                ...
                if len(tasks) >= concurrent or i == num_requests - 1:
                    ...
                    for response in responses:
                        if isinstance(response, Exception):
                            ...
                            response_time = (
                                (time.time() - batch_start) * 1000 / len(tasks)
                            )
    ...
    async def test_search_endpoint(self) -> PerformanceMetrics:
        ...
        for query in self.test_data["search_queries"][:5]:  # Test 5 queries
            ...
            metrics = await self.test_endpoint(
                "/api/forge/search", params=params, num_requests=50, concurrent=5
            )
            all_metrics.append(metrics)
            print(
                f"  Query '{query}': avg={metrics.avg_time:.2f}ms, p95={metrics.p95_time:.2f}ms"
            )

        # Aggregate metrics
        combined = PerformanceMetrics(endpoint="/api/forge/search")
        combined.avg_time = statistics.mean([m.avg_time for m in all_metrics])
        combined.p95_time = statistics.mean(
            [m.p95_time for m in all_metrics if m.p95_time > 0]
        )
        combined.requests_per_second = statistics.mean(
            [m.requests_per_second for m in all_metrics]
        )
    ...
    async def test_categories_endpoint(self) -> PerformanceMetrics:
        ...
        metrics = await self.test_endpoint(
            "/api/forge/categories", num_requests=100, concurrent=10
        )

        print(
            f"  avg={metrics.avg_time:.2f}ms, p95={metrics.p95_time:.2f}ms, rps={metrics.requests_per_second:.2f}"
        )
    ...
    async def test_tool_endpoint(self) -> PerformanceMetrics:
        ...
        for ecosystem, package in self.test_data["packages"]:
            metrics = await self.test_endpoint(
                f"/api/forge/tool/{ecosystem}/{package}", num_requests=50, concurrent=5
            )
            all_metrics.append(metrics)
    ...
    async def test_stack_endpoint(self) -> PerformanceMetrics:
        ...
        for use_case in self.test_data["use_cases"][:3]:
            json_data = {"use_case": use_case, "max_tools": 5, "min_trust_score": 70.0}
    ...
    async def simulate_load_patterns(self):
        ...
        for pattern_name, req_per_min, concurrent in patterns:
            ...
            print(
                f"  Response times: avg={metrics.avg_time:.2f}ms, p95={metrics.p95_time:.2f}ms, p99={metrics.p99_time:.2f}ms"
            )
            ...
            print(
                f"  Memory: avg={metrics.avg_memory_mb:.2f}MB, peak={metrics.peak_memory_mb:.2f}MB"
            )
        ...
        for endpoint, metrics in self.metrics.items():
            ...
            if metrics.peak_memory_mb > 100:
                bottlenecks["memory_intensive"].append(
                    {"endpoint": endpoint, "peak_memory_mb": metrics.peak_memory_mb}
                )
    ...
    async def run_comprehensive_analysis(self):
        ...
        self.metrics[
            "/api/forge/tool/{ecosystem}/{package}"
        ] = await self.test_tool_endpoint()
        ...
        if bottlenecks["slow_endpoints"]:
            ...
            for item in bottlenecks["slow_endpoints"]:
                print(
                    f"  - {item['endpoint']}: {item['avg_time_ms']:.2f}ms avg, {item['p95_time_ms']:.2f}ms p95"
                )
        ...
        if bottlenecks["low_throughput"]:
            ...
            for item in bottlenecks["low_throughput"]:
                print(
                    f"  - {item['endpoint']}: {item['requests_per_second']:.2f} req/s"
                )
        ...
        for endpoint, metrics in self.metrics.items():
            print(f"  {endpoint}:")
            print(
                f"    Response Time: avg={metrics.avg_time:.2f}ms, p95={metrics.p95_time:.2f}ms"
            )
            print(f"    Throughput: {metrics.requests_per_second:.2f} req/s")
            if metrics.failed_requests > 0:
                print(
                    f"    Error Rate: {(metrics.failed_requests / metrics.total_requests) * 100:.1f}%"
                )

        # Performance requirements check
        print("\n Performance Requirements Check:")
        requirements = {
            "API response times < 200ms": all(
                m.avg_time < 200 for m in self.metrics.values()
            ),
            "Support 100 concurrent users": self.metrics.get(
                "/api/forge/search", PerformanceMetrics("")
            ).requests_per_second
            > 100,
            "Memory usage < 2GB": all(
                m.peak_memory_mb < 2048 for m in self.metrics.values()
            ),
        }
    ...
    def export_results(self):
        ...
        filename = (
            f"forge_performance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
