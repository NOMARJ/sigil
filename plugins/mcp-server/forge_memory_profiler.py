...
class ForgeMemoryProfiler:
    ...
    def compare_snapshots(self, before: MemorySnapshot, after: MemorySnapshot) -> Dict[str, Any]:
        ...
        for alloc in after.top_allocations:
            location = alloc[0]
            if location not in before_allocs or alloc[1] > before_allocs[location][1] * 1.5:
    ...
    async def profile_endpoint_memory(self, endpoint_func, *args, **kwargs) -> Dict[str, Any]:
        ...
        if growth["heap_growth_mb"] > 10 and abs(gc_recovery["heap_growth_mb"]) < growth["heap_growth_mb"] * 0.5:
            self.memory_leaks.append({
                ...
                "potential_leak_mb": growth["heap_growth_mb"] + gc_recovery["heap_growth_mb"],
    ...
    def analyze_memory_patterns(self) -> Dict[str, Any]:
        ...
        time_span_minutes = (self.snapshots[-1].timestamp - self.snapshots[0].timestamp).total_seconds() / 60
        if time_span_minutes > 0:
            memory_growth = self.snapshots[-1].rss_mb - self.snapshots[0].rss_mb
            patterns["memory_growth_rate_mb_per_min"] = memory_growth / time_span_minutes
        ...
        for i in range(1, len(self.snapshots)):
            ...
            if curr.gc_stats["collections"][0] > prev.gc_stats["collections"][0]:
                gc_effect = {
                    ...
                    "memory_freed_mb": prev.rss_mb - curr.rss_mb if curr.rss_mb < prev.rss_mb else 0
                }
        ...
        patterns["memory_hotspots"] = sorted(hotspots, key=lambda x: x["average_size_mb"], reverse=True)[:10]
...
async def profile_forge_apis():
    ...
    classifier_profile = await profiler.profile_endpoint_memory(
        forge_classifier.forge_classifier.classify_package,
        test_input
    )
    ...
    async def test_matching():
        ...
        matches.extend(await forge_matcher.forge_matcher.find_protocol_matches("test-id"))
        matches.extend(await forge_matcher.forge_matcher.find_complementary_matches("test-id"))
        return matches
