...
- Without `awk`, the pipe check reads the options of non-shell interpreters with one shared list, where the native hook and the awk per-stage check read them per interpreter: `curl … | python3 -E` is allowed there.
