...
- Like the native hook, the fallback judges these per pipeline stage, and it follows `cd`. It no longer denies a download piped into an interpreter that reads it as data (`| python3 -m json.tool`, `| bash -c '…'`, `| sh ./script.sh`).
