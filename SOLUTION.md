...
| 2026-05-04 | claude_service is a thin wrapper over LLMService HTTP primitive (ADR-0003, **proposed**) | Smallest reversible unblock for F1.7; preserves all 33 advertised Pro routes; ~80 LOC single-file shim; trivially swappable for Branch B (refactor) or Branch C (drop bulk routes) later | Refactor bulk_analyzer to use llm_service directly (Branch B, higher churn, awkward request-shape mapping); drop bulk-analyze routes from interactive.py (Branch C, 4 routes lost AND still requires fixing feedback_processor) | Reece (pending) |
