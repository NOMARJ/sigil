...
def test_finding_mapping():
    ...
    test_findings = [
        {"rule": "install_script_detected", "severity": "high", "snippet": "setup.py install", "description": "Install script found"},
        {"rule": "eval_usage", "severity": "critical", "snippet": "eval(user_input)", "description": "Dangerous eval usage"},
        {"rule": "network_request", "severity": "medium", "snippet": "fetch('https://api.example.com')", "description": "External API call"},
        {"rule": "environment_access", "severity": "low", "snippet": "process.env.API_KEY", "description": "Environment variable access"},
        {"rule": "base64_encoded", "severity": "medium", "snippet": "atob(encoded_data)", "description": "Base64 decoding detected"},
        {"rule": "git_history", "severity": "info", "snippet": "single commit", "description": "Limited git history"},
        {"rule": "prompt_injection", "severity": "high", "snippet": "ignore previous instructions", "description": "Potential prompt injection"},
        {"rule": "skill_tampering", "severity": "medium", "snippet": "skill.yaml modified", "description": "Skill configuration modified"}
    ]
    ...
    assert len(install_hooks_phase["findings"]) > 0, "Install hooks phase should have findings"
    assert install_hooks_phase["risk_level"] != "clean", "Install hooks should be marked risky"
    
    code_patterns_phase = scan_phases[1]  # Phase 2
    assert len(code_patterns_phase["findings"]) > 0, "Code patterns phase should have findings"
    assert code_patterns_phase["risk_level"] != "clean", "Code patterns should be marked risky"
    
    prompt_injection_phase = scan_phases[6]  # Phase 7
    assert len(prompt_injection_phase["findings"]) > 0, "Prompt injection phase should have findings"
...
def test_trust_score_calculation():
    ...
    clean_phases = [
        {"phase": 1, "name": "Install Hooks", "weight": 10, "score": 10, "risk_level": "clean"},
        {"phase": 2, "name": "Code Patterns", "weight": 5, "score": 5, "risk_level": "clean"},
        {"phase": 3, "name": "Network / Exfil", "weight": 3, "score": 3, "risk_level": "clean"},
        {"phase": 4, "name": "Credentials", "weight": 2, "score": 2, "risk_level": "clean"},
        {"phase": 5, "name": "Obfuscation", "weight": 5, "score": 5, "risk_level": "clean"},
        {"phase": 6, "name": "Provenance", "weight": 3, "score": 3, "risk_level": "clean"},
        {"phase": 7, "name": "Prompt Injection", "weight": 10, "score": 10, "risk_level": "clean"},
        {"phase": 8, "name": "Skill Security", "weight": 5, "score": 5, "risk_level": "clean"},
    ]
    
    clean_trust_score = worker._calculate_trust_score(clean_phases)
    assert 90 <= clean_trust_score <= 100, f"Clean phases should have high trust score, got {clean_trust_score}"
    
    # Test with risky phases (should get lower trust score)
    risky_phases = [
        {"phase": 1, "name": "Install Hooks", "weight": 10, "score": 5, "risk_level": "high"},  # 50% score
        {"phase": 2, "name": "Code Patterns", "weight": 5, "score": 2, "risk_level": "medium"},  # 40% score
        {"phase": 3, "name": "Network / Exfil", "weight": 3, "score": 3, "risk_level": "clean"},
        {"phase": 4, "name": "Credentials", "weight": 2, "score": 2, "risk_level": "clean"},
        {"phase": 5, "name": "Obfuscation", "weight": 5, "score": 5, "risk_level": "clean"},
        {"phase": 6, "name": "Provenance", "weight": 3, "score": 3, "risk_level": "clean"},
    ...
    assert risky_trust_score < clean_trust_score, "Risky phases should have lower trust score than clean"
...
def test_capability_extraction():
    ...
    findings = [
        {"snippet": "fetch('https://api.example.com')", "description": "Network request"},
...
def test_environment_variable_extraction():
    ...
    findings = [
        ...
        {"snippet": "const token = process.env.GITHUB_TOKEN", "description": "GitHub token"}
    ]
...
def test_category_determination():
    ...
    # Test database tool
    db_category = worker._determine_category("postgres-connector", "A PostgreSQL database connector")
    assert db_category == "database_connectors", f"Expected database_connectors, got {db_category}"
    
    # Test AI tool
    ai_category = worker._determine_category("gpt-helper", "An AI assistant using GPT models")
    assert ai_category == "ai_llm_tools", f"Expected ai_llm_tools, got {ai_category}"
    
    # Test API tool
    api_category = worker._determine_category("stripe-api", "Stripe payment API integration")
    assert api_category == "api_integrations", f"Expected api_integrations, got {api_category}"
...
def test_install_command_generation():
    ...
    assert worker._generate_install_command("skills", "test-skill") == "npx skills.sh add test-skill"
    assert worker._generate_install_command("mcp", "test-mcp") == "npm install test-mcp"
    assert worker._generate_install_command("npm", "test-package") == "npm install test-package"
    assert worker._generate_install_command("pypi", "test-package") == "pip install test-package"
...
def test_security_finding_generation():
    ...
    findings = [
        {"rule": "eval_detected", "severity": "critical", "description": "Dangerous eval usage"},
        {"rule": "network_call", "severity": "medium", "description": "External network request"},
        {"rule": "info_log", "severity": "info", "description": "Informational finding"}
    ]
    
    security_findings = worker._generate_security_findings(findings)
    
    # Should only include critical and medium findings, not info
    assert len(security_findings) == 2, f"Expected 2 security findings, got {len(security_findings)}"
...
def test_enriched_data_generation():
    ...
    findings = [
        {"rule": "network_request", "severity": "low", "snippet": "fetch('/api')", "description": "API call"}
    ]
