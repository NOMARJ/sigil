...
class MSSQLForgeSetup:
    ...
    async def check_connection(self) -> bool:
        ...
        try:
            ...
            if not db.connected:
                ...
                logger.error("Expected format: Driver={ODBC Driver 18 for SQL Server};Server=...;Database=...;Uid=...;Pwd=...")
                return False
    ...
    async def create_forge_tables(self) -> bool:
        ...
        migration_file = Path(__file__).parent.parent / "migrations" / "004_create_forge_classification.sql"
    ...
    async def process_sample_data(self, limit: int = 10) -> bool:
        ...
        try:
            from api.services.forge_classifier import forge_classifier, ClassificationInput, Finding, ScanPhase, Severity
            import json
            ...
            for scan in scans:
                try:
                    ...
                    if existing:
                        logger.debug(f"Skipping {scan['ecosystem']}/{scan['package_name']} - already classified")
                        continue
                    ...
                    processed += 1
                    logger.info(f"✅ Classified {scan['ecosystem']}/{scan['package_name']} as {result.category}")
                    
                except Exception as e:
                    errors += 1
                    logger.error(f"❌ Failed to process {scan.get('package_name', 'unknown')}: {e}")
            
            logger.info(f"📈 Processing complete: {processed} classified, {errors} errors")
            return processed > 0
    ...
    async def run_setup(self, create_tables: bool, process_data: bool, data_limit: int, verify: bool):
        ...
        required_tables = ['forge_classification', 'forge_capabilities', 'forge_categories']
        ...
        if process_data:
            ...
            if scan_count == 0:
                logger.warning("⚠️ No data in public_scans table - cannot process forge data")
                logger.warning("You need to run scans first or import data from Supabase")
            else:
...
async def main():
    ...
    if not settings.database_configured:
        ...
        logger.error("export DATABASE_URL='Driver={ODBC Driver 18 for SQL Server};Server=...;Database=...;Uid=...;Pwd=...'")
        return 1
