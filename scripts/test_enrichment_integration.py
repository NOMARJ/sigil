...
def test_complete_enrichment_flow():
    ...
    metadata = {
        ...
        "license": "MIT",
        "repository": {
            "url": "https://github.com/anthropic/claude-code-assistant"
        },
    ...
    print("📋 Sample Data:")
    print(f"  Package: {scan_record['ecosystem']}/{scan_record['package_name']} v{scan_record['package_version']}")
    ...
    try:
        ...
        required_fields = [
            "id", "slug", "name", "ecosystem", "category", "description",
            "trust_score", "scan_phases", "security_findings", "capabilities",
            "environment_variables", "file_access", "network_connectivity",
            "usage_examples", "security_report"
        ]
        ...
        print(f"  Security Findings: {len(enriched_data['security_findings'])} findings")
        print(f"  Capabilities: {len(enriched_data['capabilities'])} capabilities")
        print(f"  Environment Variables: {len(enriched_data['environment_variables'])} vars")
        ...
        for phase in enriched_data['scan_phases'][:3]:
            print(f"  Phase {phase['phase']}: {phase['name']} - {phase['risk_level']} ({phase['score']}/{phase['weight']})")
...
if __name__ == "__main__":
    ...
    def _generate_enriched_data_sync(self, ecosystem, package_name, package_version, scan_record, findings, metadata):
