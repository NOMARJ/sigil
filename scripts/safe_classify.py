...
def classify_package(package_name, ecosystem, version, risk_score, findings_count, verdict):
    ...
    # LLM packages
    elif any(kw in package_name_lower for kw in ['llm', 'openai', 'anthropic', 'claude', 'gpt', 'gemini', 'langchain', 'llamaindex']):
        category = 'llm-tools'
    ...
    # Security packages
    elif any(kw in package_name_lower for kw in ['security', 'auth', 'encrypt', 'firewall', 'scanner', 'vulner', 'malware']):
        category = 'security'
    ...
    # Database packages
    elif any(kw in package_name_lower for kw in ['database', 'sql', 'mongo', 'redis', 'postgres', 'mysql', 'sqlite']):
        category = 'data'
        subcategory = 'database'
    
    # Web packages
    elif any(kw in package_name_lower for kw in ['http', 'express', 'flask', 'fastapi', 'server', 'api', 'rest', 'graphql']):
        category = 'web'
    ...
    # Crypto packages
    elif any(kw in package_name_lower for kw in ['crypto', 'bitcoin', 'ethereum', 'wallet', 'miner', 'blockchain', 'web3']):
        category = 'crypto'
    ...
    # ML packages
    elif any(kw in package_name_lower for kw in ['tensorflow', 'pytorch', 'scikit', 'pandas', 'numpy', 'ml', 'machine-learning', 'keras']):
        category = 'ml'
        subcategory = 'ml-library'
    
    # CLI tools
    elif any(kw in package_name_lower for kw in ['cli', 'command', 'terminal', 'console']):
        category = 'cli'
        subcategory = 'cli-tool'
    
    # Testing tools
    elif any(kw in package_name_lower for kw in ['test', 'jest', 'mocha', 'pytest', 'unittest']):
        category = 'testing'
