...
async def get_enrichment_stats():
    ...
    try:
        ...
        return {
            ...
            "completion_percentage": (enriched_count / total_scans * 100) if total_scans > 0 else 0,
...
async def export_pending_records(output_file: str):
    ...
    try:
        ...
        with open(output_file, 'w', newline='') as csvfile:
            fieldnames = ['ecosystem', 'package_name', 'package_version', 'verdict', 'scanned_at']
...
def print_detailed_status(detailed_status):
    ...
    for row in detailed_status:
        ...
        print(f"{ecosystem:<12} {total:<8,} {enriched:<10,} {pending:<8,} {percentage:<6.1f}%")
...
async def main():
    ...
    parser.add_argument(
        "--detailed",
        action="store_true",
        help="Show detailed breakdown by ecosystem"
    )
    parser.add_argument(
        "--export-csv",
        metavar="FILE",
        help="Export pending records to CSV file"
    )
    parser.add_argument(
        "--watch",
        action="store_true",
