...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('migration_deployment.log'),
...
class AzureMigrationDeployer:
    ...
    def detect_sql_platform(self, content: str) -> str:
        ...
        if any(keyword in content_lower for keyword in [
            'uniqueidentifier', 'nvarchar', 'datetimeoffset', 'newid()',
            'sys.tables', 'object_id(', 'sysdatetimeoffset()', 'if not exists (select * from sys.',
            'begin...end', 'print '
        ]):
            return 'tsql'
        ...
        # PostgreSQL indicators    
            ...
            'serial primary key', 'timestamptz', 'jsonb', 'generate_unsubscribe_token()',
            'plpgsql', 'returns trigger'
        ]):
            return 'postgresql'
            
        # SQLite indicators
        elif any(keyword in content_lower for keyword in [
            'datetime(\'now\')', 'insert or ignore', 'if not exists'
    ...
    def load_migrations(self) -> None:
        ...
        for config in migration_configs:
            ...
            try:
                ...
                logger.info(f"Loaded migration: {migration.name} ({migration.platform})")
    ...
    def log_migration_statuses(self, conn, migrations: List[Migration]) -> None:
        ...
        for migration in migrations:
            status = "APPLIED" if self.is_migration_applied(conn, migration.name) else "PENDING"
            logger.info(f"  {migration.order}. {migration.name} [{migration.platform}] - {status}")
    ...
    def execute_migration(self, conn, migration: Migration) -> bool:
        ...
        try:
            ...
            for i, block in enumerate(go_blocks):
                ...
                requires_autocommit = any(keyword in statement.upper() for keyword in [
                    'CREATE FULLTEXT CATALOG', 'DROP FULLTEXT CATALOG',
            ...
            cursor.execute(
                ...
                """,
                migration.name, migration.name, migration.name
            )
            conn.commit()
            ...
            try:
                cursor.execute(
                    ...
                    """,
                    migration.name, str(e), migration.name, migration.name, str(e)
                )
                conn.commit()
            except:
    ...
    def deploy_migrations(self, dry_run: bool = False, status_only: bool = False) -> None:
        ...
        try:
            ...
            if status_only:
                logger.info("Status check complete (--status-only). No migrations were applied.")
                return
            ...
            for migration in tsql_migrations:
                if self.is_migration_applied(conn, migration.name):
                    logger.info(f"⏩ Skipping already applied migration: {migration.name}")
                    continue
                
                logger.info(f"🔄 Deploying migration {migration.order}: {migration.name}")
                
                if self.execute_migration(conn, migration):
                    successful += 1
                else:
                    failed += 1
                    logger.error(f"Migration {migration.name} failed - stopping deployment")
                    break
...
def main():
    ...
    if not database_url:
        ...
        try:
            ...
            ], cwd='../sigil-infra/azure', capture_output=True, text=True)
    ...
    if not database_url:
        logger.error("DATABASE_URL not found. Set environment variable or run from correct directory.")
        sys.exit(1)
    ...
    logger.info(f"Database: {database_url.split('Server=')[1].split(',')[0] if 'Server=' in database_url else 'Unknown'}")
