...
class EnrichmentController:
    ...
    def __init__(self, dry_run: bool = False, batch_size: int = None, max_records: int = None):
    ...
    async def run_dry_run(self):
        ...
        for i, record in enumerate(pending_records[:5]):
            logger.info(f"  {i+1}. {record['ecosystem']}/{record['package_name']} v{record.get('package_version', 'latest')}")
    ...
    async def run_continuous(self):
        ...
        while self.running and records_processed < self.worker.max_records:
            try:
                ...
                batch_records = pending_records[:min(self.worker.batch_size, remaining_budget)]
                ...
                logger.info(f"✅ Processed {records_processed}/{self.worker.max_records} records")
                
                # Sleep between batches
                if records_processed < self.worker.max_records and len(pending_records) > len(batch_records):
        ...
        if records_processed >= self.worker.max_records:
            logger.info(f"✅ Reached max records limit ({self.worker.max_records}), stopping")
        else:
