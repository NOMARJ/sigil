...
def classify_package(scan):
    ...
    # Skills packages
    elif 'skill' in package_name or 'clawbot' in package_name or 'clawhub' in package_name:
        category = 'skills'
    ...
    # AI/Agent packages
    elif any(keyword in package_name for keyword in ['agent', 'assistant', 'bot', 'ai-']):
        category = 'ai-agents'
        subcategory = 'agent-tool'
    
    # LLM packages
    elif any(keyword in package_name for keyword in ['llm', 'openai', 'anthropic', 'claude', 'gpt', 'gemini']):
        category = 'llm-tools'
        subcategory = 'llm-integration'
    
    # Security packages
    elif any(keyword in package_name for keyword in ['security', 'auth', 'encrypt', 'secure', 'firewall']):
        category = 'security'
        subcategory = 'security-tool'
    
    # Database packages
    elif any(keyword in package_name for keyword in ['database', 'sql', 'mongo', 'redis', 'postgres']):
        category = 'data'
        subcategory = 'database'
    
    # Web/HTTP packages
    elif any(keyword in package_name for keyword in ['http', 'express', 'flask', 'fastapi', 'server', 'api']):
        category = 'web'
        subcategory = 'web-framework'
    
    # Crypto packages (high risk)
    elif any(keyword in package_name for keyword in ['crypto', 'bitcoin', 'ethereum', 'wallet', 'miner']):
        category = 'crypto'
...
def main():
    ...
    for scan in scans:
        try:
            ...
            if classified % 100 == 0:
                conn.commit()
                print(f"Progress: {classified} classified, {skipped} skipped, {errors} errors")
    ...
    for row in cursor.fetchall():
        print(f"  {row.category}: {row.count} packages (avg confidence: {row.avg_confidence:.2f})")
