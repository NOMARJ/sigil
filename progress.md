...
- **Status:** PARTIAL — operator-gated. Re-attempted 2026-05-04 AND 2026-06-21 (NOM-884, branch `claude/admiring-hopper-0q4dj4`). Both attempts confirm same three blockers: (1) Stripe MCP `GetWebhookEndpoints` not available in this environment; (2) local Stripe CLI configured for wrong account (`acct_1TNsZTFvlPr69lA2`, not Sigil/NOMARK `acct_1RkD8NFhPhxEz27f`); (3) CHARTER II.5 forbids reading Azure Container App secrets into agent context. Evidence file `evidence/F-003/US-105a-test-mode-webhook-audit.md` captures the runbook (Options A/B/C); verbatim `enabled_events` capture + any fix application require operator.
