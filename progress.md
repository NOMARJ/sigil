...
- **CI Dashboard build (2026-05-03, /bugfix all):** FIXED in commit `9b13983`. `dashboard/package-lock.json` was out of sync with `package.json` (missing `posthog-js` + ~30 transitive deps including `@opentelemetry/*`, `@posthog/core`, `protobufjs`, `dompurify`). `npm ci` in CI failed every run since at least 2026-04, which means **every prior /bugfix commit (F2 = 2a72827, F1.5 = 69b9f13, F1.6 = 43f2165) was deployed manually via `az containerapp update`**, not via the workflow. Regenerated lock via `npm install`. `npm ci --dry-run` clean; `pnpm exec tsc --noEmit` clean (the pre-existing posthog-js typecheck error is also resolved as a side-effect — the dep was never actually installed). 21/21 dashboard Jest tests still pass. After this commit, future API + dashboard pushes auto-deploy via the workflow. See `evidence/F-003/CI-dashboard-build-fixed.md`.
