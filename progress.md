...
- **F1.7 NEW finding (does NOT block STORY-104):** `interactive.router` is NOT imported or registered in `api/main.py:295-359` despite defining 33 Pro-gated routes (the AI investigation, chat, attack-chain, compliance-mapping endpoints — Sigil Pro's headline features). Every advertised Pro feature returns 404 in production. Effect: paid Pro users will hit 404s when they try to use what they paid for. P0 for actual Pro feature delivery, not for STORY-104's gating-contract baseline (`/v1/policies` and `/v1/threats` are also Pro-gated and registered, served the 403). Fix: add `interactive` to the `from api.routers import (...)` block + `app.include_router(interactive.router)`.
