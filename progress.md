...
- **CI deploy dispatch (NEW — exposed by the build-step fix above):** **operator-only**. Run `25277397051` for commit `9b13983` revealed the deeper layer: `Trigger Infrastructure Deployment` step fails with `Bad credentials` on `peter-evans/repository-dispatch@v3` calling `NOMARJ/sigil-infra`. The `secrets.INFRA_REPO_PAT` token is invalid/expired. Last successful end-to-end deploy was **2026-03-17** (`99c46af`), ~7 weeks ago — explains the entire F-003 config-drift cascade (F1 Auth0 env, STORY-100 annual Prices, F2 dead routes, F1.6 SQL keyword bug — all would have been caught earlier with working auto-deploy). Operator needs to rotate `INFRA_REPO_PAT` (fine-grained PAT, `Contents: Read & write` on `NOMARJ/sigil-infra`) and re-run the failed job: `gh run rerun 25277397051 --failed`. See `evidence/F-003/CI-dashboard-build-fixed.md` § Operator Action Required.
