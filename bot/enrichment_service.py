#!/usr/bin/env python3
"""
Forge Enrichment Service Entry Point
...
This module is designed to run within the Docker container and provides
the same functionality as scripts/enrichment_worker_service.py but within
the bot module structure.
"""
...
async def health_check():
    ...
    try:
        ...
        status = "healthy" if db_connected and worker_state["status"] in ["running", "idle"] else "unhealthy"
        ...
        return {
            ...
            "uptime_seconds": int((datetime.utcnow() - worker_state["started_at"]).total_seconds())
        }
...
async def get_metrics():
    ...
    try:
        ...
        completion_percentage = (enriched_records / total_records * 100) if total_records > 0 else 0
        
        return {
            "worker_metrics": {
                "status": worker_state["status"],
                "started_at": worker_state["started_at"].isoformat(),
                "last_run": worker_state["last_run"].isoformat() if worker_state["last_run"] else None,
...
async def run_enrichment_worker():
    ...
    try:
        ...
        logger.info(f"  Delay between batches: {enrichment_worker.delay_between_batches}s")
        
        await db.connect()
        
        while not shutdown_event.is_set() and worker_state["processed_records"] < enrichment_worker.max_records:
            try:
                worker_state["status"] = "running"
                ...
                remaining_budget = enrichment_worker.max_records - worker_state["processed_records"]
                batch_records = pending_records[:min(enrichment_worker.batch_size, remaining_budget)]
                
                worker_state["current_batch"] += 1
                worker_state["total_batches"] = worker_state["current_batch"] + max(0, (len(pending_records) - len(batch_records)) // enrichment_worker.batch_size)
                
                logger.info(f"Processing batch {worker_state['current_batch']} with {len(batch_records)} records...")
                
                # Process batch
                await enrichment_worker._process_batch(batch_records)
                
                worker_state["processed_records"] += len(batch_records)
                logger.info(f"✅ Processed {worker_state['processed_records']}/{enrichment_worker.max_records} records")
                
                # Sleep between batches if there are more records and we haven't hit the limit
                if (worker_state["processed_records"] < enrichment_worker.max_records and 
        ...
        if worker_state["processed_records"] >= enrichment_worker.max_records:
            ...
            logger.info(f"✅ Reached max records limit ({enrichment_worker.max_records}), stopping")
        else:
