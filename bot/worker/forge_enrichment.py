...
class ForgeEnrichmentWorker:
    ...
    async def _enrich_single_record(self, scan_record: Dict[str, Any]) -> None:
        ...
        enriched_data = await self._generate_enriched_data(
            ecosystem, package_name, package_version, scan_record, findings, metadata
        )
    ...
    async def _generate_enriched_data(
        ...
        related_tools = await self._find_related_tools(
            ecosystem, package_name, capabilities
        )
    ...
    def _map_to_eight_phases(
        ...
        for finding in findings:
            ...
            if 0 <= phase_idx < 8:
                ...
                phases[phase_idx]["score"] = max(
                    0, phases[phase_idx]["score"] + severity_impact
                )

                # Update risk level
                if severity in ["critical", "high"]:
                    phases[phase_idx]["risk_level"] = (
    ...
    def _extract_capabilities(
        ...
        if any(
            term in description for term in ["ai", "llm", "gpt", "claude", "assistant"]
        ):
            capabilities.append(
                {
                    "type": "ai",
    ...
    def _extract_network_connectivity(
        self, findings: List[Dict[str, Any]], metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_file_access_patterns(
        self, findings: List[Dict[str, Any]], metadata: Dict[str, Any]
    ) -> List[str]:
    ...
    def _generate_usage_examples(
        self, ecosystem: str, package_name: str, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _find_related_tools(
        self, ecosystem: str, package_name: str, capabilities: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
            ...
            for result in results[:3]:  # Limit to 3 related tools
                try:
                    ...
                    related_tools.append(
                        {
                            "id": self._generate_tool_uuid(
                                result["ecosystem"], result["package_name"]
                            ),
                            ...
                            "package_url": self._generate_package_url(
                                result["ecosystem"], result["package_name"]
                            ),
                            "tags": [],
    ...
    def _generate_security_report(
        self, scan_phases: List[Dict[str, Any]], security_findings: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Generate comprehensive security report."""
        # Calculate overall risk
        high_risk_phases = [
            p for p in scan_phases if p["risk_level"] in ["high", "critical"]
        ]
        ...
        if high_risk_phases:
            ...
            summary = (
                "This tool passed security analysis with no significant risks detected."
            )
        ...
        if overall_risk == "HIGH_RISK":
            recommendations.insert(
                0, "Consider alternative packages with better security profiles"
            )
            ...
            "compliance_checks": [
                {
                    ...
                    "passed": overall_risk != "HIGH_RISK",
                    "details": "Based on static analysis",
                },
                {
                    "standard": "Supply Chain Security",
                    "passed": True,
                    "details": "Package scanned and analyzed",
                },
            ],
        }
    ...
    def _get_finding_impact(self, finding: Dict[str, Any]) -> str:
        ...
        if severity == "critical":
            return (
                "Critical security vulnerability that could compromise system security"
            )
        elif severity == "high":
    ...
    def _get_finding_recommendation(self, finding: Dict[str, Any]) -> str:
        ...
        if "network" in rule:
            ...
            return (
                "Review code execution patterns for potential injection vulnerabilities"
            )
        else:
    ...
    def _determine_category(self, package_name: str, description: str) -> str:
        ...
        if any(
            term in name_lower
            for term in ["db", "database", "postgres", "mysql", "redis", "mongo"]
        ):
            return "database_connectors"
        elif any(
            term in desc_lower
            for term in ["database", "sql", "postgres", "mysql", "redis"]
        ):
            return "database_connectors"
        elif any(
            term in name_lower for term in ["ai", "gpt", "llm", "chat", "assistant"]
        ):
            return "ai_llm_tools"
    ...
    async def _save_enriched_classification(
        self, scan_record: Dict[str, Any], enriched_data: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            classification_data = {
                ...
                "risk_indicators": json.dumps(
                    [f["description"] for f in enriched_data["security_findings"]]
                ),
            ...
            if existing:
                ...
                await db.update(
                    "forge_classification", {"id": existing["id"]}, classification_data
                )
            ...
            if classification_id and enriched_data["capabilities"]:
                ...
                await db.delete(
                    "forge_capabilities", {"classification_id": classification_id}
                )
