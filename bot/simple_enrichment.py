...
async def main():
    ...
    try:
        ...
        while not shutdown_event.is_set() and processed_records < worker.max_records:
            try:
                logger.info(f"Looking for pending enrichments... ({processed_records}/{worker.max_records} processed)")
                ...
                batch_records = pending_records[:min(worker.batch_size, remaining_budget)]
                ...
                logger.info(f"✅ Processed {processed_records}/{worker.max_records} records")
                
                # Sleep between batches if there are more records and we haven't hit the limit
                if (processed_records < worker.max_records and 
        ...
        if processed_records >= worker.max_records:
            logger.info(f"✅ Reached max records limit ({worker.max_records}), stopping")
        else:
